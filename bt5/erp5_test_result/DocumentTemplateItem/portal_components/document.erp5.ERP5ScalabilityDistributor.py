...
# XXX computer guid only used in dev sessions (testnode running on a webrunner)
DEV_COMPUTER = "COMP-2732"
...
class ERP5ScalabilityDistributor(ERP5ProjectUnitTestDistributor):
  ...
  def _getAvailableNodes(self, test_suite_specialise_title, portal):
    ...
    if len(available_nodes) == 0: return []
    # XXX for dev mode, the node is a 'slaprunner' (replace it for dev computer)
    available_nodes = [ node.getComputerGuid() for node in available_nodes
                          if node.getComputerGuid() != "slaprunner" ]
    if len(available_nodes) == 0: available_nodes.append(DEV_COMPUTER)

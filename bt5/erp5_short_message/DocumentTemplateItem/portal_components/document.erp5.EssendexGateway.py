...
class EssendexGateway(XMLObject):
  ...
  def send(self, text, recipient, sender):
    ...
    if result['Result'] == "OK":
      ...
      raise_(SMSGatewayError, urllib.unquote(result.get('Message', "Impossible to send the SMS")))
  ...
  def getMessageStatus(self, message_id):
    ...
    if result['Result'] == "OK":
      ...
      #we get an error when call the gateway
      raise_(SMSGatewayError, urllib.unquote(result.get('Message', "Impossible to get the message status")))
  ...
  def receive(self, REQUEST, **kw):
    ...
    try:
      ...
      if xml['AccountId'] != self.getGatewayAccountId():
        raise_(Unauthorized, 'Bad accound id (%s)' % xml['AccountId'])
      ...
      if notification_type == 'InboundMessage':
        ...
        raise_(SMSGatewayError, "'MessageError' notification is not implemented (%s)" % str(kw))
      elif notification_type == 'SubscriptionEvent':
        raise_(SMSGatewayError, "'MessageError' notification is not implemented (%s)" % str(kw))
      else:
        raise_(SMSGatewayError, "Unknow '%s' notification (%s)" % (notification_type, str(kw)))
    finally:
      #Restore orinal user
  ...
  def pullLastMessageList(self, start_date=None, stop_date=None):
    ...
    if result['Result'] == "OK":
      ...
      #we get an error when call the gateway
      raise_(SMSGatewayError, urllib.unquote(result.get('Message', "Impossible to get last message list")))

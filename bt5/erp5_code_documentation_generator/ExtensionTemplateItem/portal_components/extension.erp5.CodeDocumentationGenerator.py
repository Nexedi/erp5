...
def ERP5Site_generateCodeDocumentation(self,
  ...
  if backend == 'sphinx':
    def filterMemberToDocument(member):
      if not inspect.isclass(member):
        return False
      return member.__module__ in module_name_list
...
html_theme = 'default'
master_doc = 'index'
extensions = ['sphinx.ext.autodoc', 'sphinx.ext.autosummary', 'sphinx.ext.doctest', 'sphinx.ext.viewcode']
...
""".format(module_name))
          ...
          for clsname, _ in inspect.getmembers(module, filterMemberToDocument):
            title = '{} class'.format(clsname)
            index.write(title + '\n')
            index.write('-' * len(title) + '\n')
            index.write("""
.. autoclass:: {}

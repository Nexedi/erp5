...
class PersonNode(ZencartNode):
  ...
  def _asXML(self):
    ...
    try:
      ...
      # Missing mapping
      LOG("ZencartBrain.PersonNode._asXML", ERROR, "Getting category for %s raise with msg = %s" %(ValueError, msg))
      #return None
    """
    ...
    if getattr(self, "is_delivery", None) is not None:
      element = etree.SubElement(node, 'category')
      element.text = 'role/zencart_delivery'
...
class Organisation(ZencartNode):
...
class Transaction(TransactionBrain):
  ...
  def _asXML(self):
    ...
    # Add Discount
    if getattr(self,'discount_price',0) > 0:
    ...
    # Add delivery
    if getattr(self,'delivery_price',0) > 0:

...
if typing.TYPE_CHECKING:
  ...
  from erp5.component.document.OpenAPITypeInformation import OpenAPIOperation
  from erp5.component.document.OpenAPITypeInformation import OpenAPIParameter
...
class OpenAPIService(XMLObject):
  ...
  def getMatchingOperation(self, request):
    ...
    # Compute the relative part request path by removing the
    # part corresponding to the context, which can be in a
    # web site/web section 
    context_part_count = len(self.getPhysicalPath())
    web_section = self.getWebSectionValue()
    if web_section is not None:
      # remove 1 for the leading slash in web section path.
      context_part_count += len(web_section.absolute_url_path().split('/')) - 1
    request_path_parts = request.physicalPathFromURL(
      request.get('URL'), )[context_part_count:]

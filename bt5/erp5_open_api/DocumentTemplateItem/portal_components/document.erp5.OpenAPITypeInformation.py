...
class OpenAPIParameter(dict):
  ...
  def getValue(self, request):
    ...
    if not param_value:
      ...
      param_value = self.get('default', param_value)
    else:
...
class OpenAPITypeInformation(ERP5TypeInformation):
  ...
  def getOpenAPIOperationIterator(self):
    ...
    for path, path_item in six.iteritems(schema['paths']):
      # BBB py2 yield from
  ...
  def getMethodMappingValueList(self, *args, **kw):
    try:
      return self._getMethodMappingValueList(*args, **kw)
    except Exception:
      return []

  def _getMethodMappingValueList(self, *args, **kw):

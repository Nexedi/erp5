...
class Transaction(TransactionBrain):
  ...
  def _asXML(self):
    ...
    self.title = "%s %s" % (self.reference,self.title)
    ...
    if getter_line_method is not MARKER:
      ...
      for line in getter_line_method(**parameter_kw):
        key_list = ['title', 'reference', 'quantity', 'gross_price']
        ...
        #  continue
          #movement_dict['quantity'] = 1
        movement_dict['price'] = float(movement_dict['gross_price'])
        LOG("asXML returns -- movement_dict %s" %(movement_dict,), 300, "")
    ...
    for movement_dict in movement_list:
      ...
      tag_list = ('title', 'reference', 'quantity', 'price')
      for tag in tag_list:

...
class TestStaticWebSectionRedirection(ERP5TypeTestCase):
  ...
  def runTestRedirect(self, source_path, expected_failure=None, use_moved_temporarily=None, **kw):
    ...
    redirect_url = website.getProperty("redirect_domain")
    ...
    status_to_assert = MOVED_PERMANENTLY
    if use_moved_temporarily:
      status_to_assert = MOVED_TEMPORARILY

    if expected_failure:
      expectedFailure(self.assertEquals)(response.status, status_to_assert)
      expectedFailure(self.assertEquals)(response.getheader(LOCATION), redirect_location)
    else:
  ...
  def test_queryStringIgnoreLayout(self):
    self.runTestRedirect("?ignore_layout=1", expected_failure=True)

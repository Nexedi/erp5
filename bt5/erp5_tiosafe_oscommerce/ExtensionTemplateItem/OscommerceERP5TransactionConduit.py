...
class OscommerceERP5TransactionConduit(TioSafeBaseConduit):
  """
  ...
  def visitArrow(self, document=None, xml=None, **kw):
    ...
    for subnode in xml.getchildren():
      ...
      if tag in arrow_dict:
        ...
        LOG("AUREL", 300, "will set %s on %s to value %s" %(tag, document, link_object))
  ...
  def visitMovement(self, document=None, xml=None, **kw):
    ...
    if len(movement_dict_value['category']):
      ...
      LOG("AUREL", 300, "setting the variation on %s to value %s" %(sale_order_line.getPath(), variation_category_list))
      ...
      sale_order_line.setVariationCategoryList([])

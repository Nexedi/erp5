...
_OAUTHLIB_EXCEPTION_DICT = {
  ...
  for error_klass in oauthlib_errors.__dict__.itervalues()
  if (
...
class OAuth2AuthorisationClientConnector(
  ...
  def _callOAuth2(
    ...
    environ = {
      ...
      for key, value in dict(header_dict).iteritems()
    }
  ...
  def _getStateCookieDict(self, REQUEST, RESPONSE):
    ...
    for name, value in self._getRawStateCookieDict(REQUEST).iteritems():
      try:
        result[name] = decrypt(
  ...
  def login(
    ...
    try:
      ...
      name, identifier = next(self._getStateCookieDict(
        REQUEST=REQUEST,
        RESPONSE=RESPONSE,
      ).iteritems())

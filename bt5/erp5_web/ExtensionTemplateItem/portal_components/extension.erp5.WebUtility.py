...
def parseCssForUrl(text):
  ...
  return tuple list like: [
    ("data", ""),
    ("comment", "/* set body background image */"),
  ...
  """
  result = []
  parts = partition(text, re.compile(r"/\*((?:[^\*]|\*[^/])*)\*/"))  # filter comments
  #urlre = re.compile(r"""(:[ \t]*url\()((")([^"]*)"|(')([^']*)'|([^\)]*))\)""")
  urlre = re.compile(r"""(:[ \t]*url\()(\s*(")([^"]*)"\s*|\s*(')([^']*)'\s*|([^\)]*))\)""")
  i = 0
  for part in parts:
    i += 1
    if i % 2 == 0:  # comment
      result.append(("comment", part[0], parts[1]))
    else:  # non comment
      parts = partition(part[0], urlre)
      data = ""
      j = 0
      for part in parts:
        j += 1

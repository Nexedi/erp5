...
_twemoji_zip_cache = {}
def getTwemojiZip():
  if "zip" not in _twemoji_zip_cache:
    zip_file = None
    try:
      obj = portal.restrictedTraverse("twemoji/twemoji-svg.zip")
      data = bytes(obj.data or b"") if hasattr(obj, "data") else (getattr(obj, "getData", lambda: b"")() or b"")
      zip_file = zipfile.ZipFile(io.BytesIO(data))
    except (KeyError, Unauthorized, zipfile.BadZipFile):
      zip_file = None
    _twemoji_zip_cache["zip"] = zip_file
  return _twemoji_zip_cache["zip"]
...
def handleEmojiText(text):
  def replaceEmoji(match):
    ...
    if zip_file is None:
      return emoji_text
    try:
      data = zip_file.read("%s.svg" % emojiToCodepointString(emoji_text))
    except KeyError:
      return emoji_text
    return u'<img class="emoji" alt="%s" src="data:image/svg+xml;base64,%s" />' % (
...
def replaceFromDataUri(data_uri, replacer):
  ...
  if ";base64" in header:
    is_base64 = True
    data = bytes2str(b64decode(data))
  data = replacer(data)
  return "%s,%s" % (header, bytes2str(b64encode(str2bytes(data))) if is_base64 else data)

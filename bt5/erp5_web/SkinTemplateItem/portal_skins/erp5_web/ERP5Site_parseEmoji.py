...
def emojiToCodepointStringList(cluster):
  # twemoji keeps VS16 in the file name of a ZWJ sequence and drops it
  # everywhere else
  # so return both spellings and let the caller use the one
  # the archive actually has:
  #   1F3CC FE0F 200D 2640 FE0F -> 1f3cc-fe0f-200d-2640-fe0f exists  --> as_is path
  #   0031 FE0F 20E3            -> 31-20e3 exists, 31-fe0f-20e3 does not  --> without vs16 path
  as_is = "-".join("%x" % ord(c) for c in cluster)
  without_vs16 = "-".join(
    "%x" % ord(c) for c in cluster if c != VARIATION_SELECTOR_16)
  if without_vs16 == as_is:
    # no VS16 in the cluster, both spellings would be the same lookup
    return [as_is]

...
class TestQualityAssurance(ERP5TypeTestCase):
  ...
  def test_quality_element_workflow(self):
    ...
    for portal_type in self.quality_element_type:
      ...
      self.assertEqual(quality_element.getValidationState(), 'draft')
      quality_element.plan()
      self.assertEqual(quality_element.getValidationState(), 'queued')
      quality_element.confirm()
      self.assertEqual(quality_element.getValidationState(), 'expected')
      ...
      self.assertEqual(quality_element.getValidationState(), 'pending_update')
      quality_element.post()
      self.assertEqual(quality_element.getValidationState(), 'posted')
      quality_element.archive()
      self.assertEqual(quality_element.getValidationState(), 'archived')
    ...
    self.assertEqual(quality_control.getValidationState(), 'archived')
    element_list = []
    index = 0
    po = self.portal.production_order_module.newContent(
      portal_type='Production Order',
    ...
    for index in [0, 1,3, 4]:
      self.assertEqual(element_list[index].getValidationState(), 'expected')
    for index in [5, 6]:
      self.assertEqual(element_list[index].getValidationState(), 'queued')
  ...
  def test_quality_control(self):
    ...
    self.assertEqual(len(quality_element_list), 17, quality_element_list)
    quality_element_list = me_execution.Delivery_getUpcomingQualityControlOperationList()
    self.assertEqual(len(quality_element_list), 3)
    for quality_element in quality_element_list:
      self.assertEqual(quality_element.getValidationState(), 'expected')
    self.assertEqual(quality_element_list[-1].getPortalType(), 'Gate')
    ...
    self.assertEqual(quality_control.getQualityAssurance(), 'result/ok')
    ...
    self.tic()
    self.assertEqual(cloned_one.getValidationState(), 'archived')
    self.assertEqual(cloned_cloned_one.getValidationState(), 'posted')
    defect_one = cloned_cloned_one.QualityControl_RequestNewQualityControl(batch=True)

    self.assertEqual(defect_one.getValidationState(), 'expected')
  ...
  def test_gate(self):
    ...
    while True:
      ...
      for i in quality_element_list:
        self.assertEqual(i.getValidationState(), 'expected')
      ...
      gate.Gate_validate()
      self.tic()
      self.assertEqual(gate.getValidationState(), 'posted')
  ...
  def test_SMON(self):
    ...
    self.tic()
    self.assertEqual(SMON.getValidationState(), 'posted')
    self.assertEqual(me_execution.getSimulationState(), 'stopped')
  ...
  def test_ACOM(self):
    ...
    ACOM.ACOM_validate()
    self.tic()
    self.assertEqual(ACOM.getValidationState(), 'expected')
    ...
    self.tic()
    incompleted_list = ACOM.Base_getIncompletedOperationList()
    self.assertEqual(len(incompleted_list), 0)
    ACOM.ACOM_validate()
    self.tic()
    self.assertEqual(ACOM.getValidationState(), 'posted')
    self.assertEqual(me_execution.getSimulationState(), 'delivered')
  ...
  def _test_SMON_ACOM_operation_list(self, ACOM = False):
    ...
    for element in incompleted_list:
      ...
      self.assertEqual(element.getValidationState(), 'expected')
    ...
    self.assertEqual([x.getObject() for x in incompleted_list], [x.getObject() for x in new_incompleted_list])
    self.assertEqual(new_incompleted_list[0].getValidationState(), 'posted')
  ...
  def test_traceability(self):
    ...
    self.assertEqual(len(traceability_list), 3)
...
''')
    ...
    self.assertEqual(traceability_list[0].getReference(), '00519267020')
    me_execution.ManufacturingExecution_processTraceabilityData('''00463429720111111111111111
''')
    self.tic()
    traceability_list = me_execution.Base_getExpectedTraceabilityInputList()
...
''')
    self.tic()
    traceability_list = me_execution.Base_getExpectedTraceabilityInputList()
    self.assertEqual(len(traceability_list), 0)
...
''')
    ...
    self.assertEqual(traceability.getAggregateValue(portal_type='Serial Number'), serial_number)

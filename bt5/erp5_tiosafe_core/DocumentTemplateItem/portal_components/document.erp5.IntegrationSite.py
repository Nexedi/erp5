  ...
  def getCategoryFromMapping(self, category, product=None, create_mapping=False,
    ...
    for cat in category.split('/'):
        ...
        if current_object.getPortalType() == "Integration Base Category Mapping":
          if create_mapping_line is False:
            ...
            if missing_mapping or current_object.getDestinationReference() is None:
              ...
              raise_(ValueError, "Mapping not defined for %s" % category)
          ...
          if create_mapping:
            ...
            raise_(ValueError, "Mapping object for %s not found" %(cat,))
      ...
      if mapped_category in ("", None) and cat_object.getPortalType() == "Integration Category Mapping":
        ...
        raise_(ValueError, "Mapping not defined for %s" % cat)
...
##                                                                                        category))
  ...
  def getMappingFromCategory(self, category):
    ...
    if len(mapping) != 1:
      raise_(IndexError, 'The integration base category mapping %s must be mapped and with only one base_category' % (base_category))
    ...
    if len(mapping_line) > 1:
      raise_(IndexError, 'The integration category mapping %s must be mapped with only one category' % (variation))
    try:
      # shared variation
      return '/'.join(
  ...
  def getMappingFromProperty(self, base_mapping, property_name):
    ...
    if len(mapping_line) > 1:
      raise_(IndexError, 'The integration property mapping %s must be mapped with only one category' % (property_name))

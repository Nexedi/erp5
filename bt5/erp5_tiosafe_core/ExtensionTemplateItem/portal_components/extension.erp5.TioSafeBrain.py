  ...
  def _getSynchronizationObjectList(self, sync_type, object_type_list):
    ...
    def cached_getSynchronizationObjectList(sync_type, object_type):
      ...
      if not len(module_list):
        raise_(ValueError, "Impossible to find a module for %s" %(object_type))
      ...
      for module in module_list:
        ...
        if (sync_type == "erp5") ^ (source_portal_type == "Integration Module"):
          ...
          raise_(ValueError, "Impossible to find pub/sub related to Integration Module, pub = %s with source = %s, sub = %s with source = %s" %(
    ...
    if getattr(self, 'gid', None) is None:
      ...
      for prop in property_list:
        try:
          ...
          raise_(AttributeError, "The brain doesn't have the property %s" % prop)
        gid += " %s" %(prop_value,)
    ...
    if self.object_type == "Person":
      if getattr(self, "relation", None) is not None:
        ...
        if organisation_gid is None:
          raise_(ValueError, "Impossible to find organisation for id %s on node %s" %(self.relation, self.path))
      else:
        organisation_gid = ""
      # set the tag
      if organisation_gid:
    ...
    def cached_buildVATDict():
      ...
      while len(trade_condition.contentValues(portal_type="Trade Model Line")) == 0:
        ...
        if specialized_trade_condition is None or specialized_trade_condition.getPortalType() == "Business Process":
          raise_(ValueError, 'Impossible to find a trade condition containing VAT lines, last trade condition was %s, parent was %s' %(specialized_trade_condition, trade_condition))
        else:
        ...
        for line in getter_line_method(**parameter_kw):
          ...
          if not line_type:
            ...
            if line_type == 'Discount':
              ...
              raise_(ValueError, 'Try to work on "%s" which is an invalid type.' % line_type)

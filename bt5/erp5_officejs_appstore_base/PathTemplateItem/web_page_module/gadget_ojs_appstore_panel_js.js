    ...
    .onStateChange(function (modification_dict) {
      ...
      if (modification_dict.hasOwnProperty("editable")) {
        queue
          ...
          .push(function () {
            return RSVP.all([
              gadget.getUrlFor({command: 'display', options: {page: "history"}}),
              gadget.getUrlFor({command: 'display', options: {page: "front_ojs_appstore"}}),
              gadget.getUrlFor({command: 'display', options: {page: "logout"}})
            ]);
          })
          .push(function (result_list) {
            return context.translateHtml(
              panel_template_body_list({
                "history_href": result_list[0],
                "front_href": result_list[1],
                "logout_href": result_list[2]
              })
            );
          })

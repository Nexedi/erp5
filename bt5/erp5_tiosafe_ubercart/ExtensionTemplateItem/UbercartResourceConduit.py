...
class UbercartResourceConduit(TioSafeResourceConduit):
  ...
  def _createContent(self, xml=None, object=None, object_id=None, sub_object=None,
    ...
    new_id = object.product_module.createProduct(**keyword)
    # XXX-AUREL : this must be changed to use gid definition instead
    if not new_id:
      new_id = object.IntegrationSite_lastProductID()[0].getId()
    # create the full variations in the integration site
    if variation_dict:
      for base_category, variation_list in variation_dict.items():
  ...
  def _deleteContent(self, object=None, object_id=None):
    ...
    delete_method_id = "deleteProduct" 
  ...
  def _updateXupdateUpdate(self, document=None, xml=None, previous_xml=None, **kw):
    ...
    try:
      previous_value = previous_xml[0].text.encode("utf-8")
    except IndexError:
    ...
    if tag.split('[')[0] == 'category':
      ...
      # categories
      conflict_list += self.updateCategory(document, xml, previous_value, new_value)
    else:
      # getter used to retrieve the current values and to check conflicts
      property_list = ['title', 'reference','sale_price', 'purchase_price', 'ean13']
      ...
      if current_value not in [new_value, previous_value]:
        ...
        #LOG("mayoro update product", 0, "update keyword=%s" % keyword)
  ...
  def updateCategory(self, document=None, xml=None, previous_value=None, new_value=None):
    ...
    if previous_value != "" and new_value != "" and previous_value != new_value:
      mapping = integration_site.getMappingFromCategory(previous_value)
      base_category, variation = mapping.split('/', 1)
    ...
    for subnode in xml.getchildren():
      ...
      if new_tag == 'category':
        ...
        external_axe_list = [
            ...
            for x in document.context.getProductCategoryList(product_id=document.getId())
  ...
  def _updateXupdateDel(self, document=None, xml=None, previous_xml=None, **kw):
    ...
    if tag.split('[')[0] == 'category':
      ...
      current_value = etree.XML(document.asXML()).xpath(tag)[0].text.encode("utf-8")
      current_value = integration_site.getMappingFromCategory(current_value)

      # work on variations
      variation_brain_list = document.context.product_module.getProductCategoryList(
                                      product_id=document.getId(),   
                                  )
      for brain in variation_brain_list:
        if brain.category.encode("utf-8") == current_value and previous_value == current_value:

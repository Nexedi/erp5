...
class PersonNode(UbercartNode):
  ...
  def _asXML(self):
    ...
    if getattr(self, "is_delivery", None) is not None:
      element = etree.SubElement(node, 'category')
      element.text = 'role/ubercart_delivery'
...
class Organisation(UbercartNode):
  ...
  def _asXML(self):
    ...
    self._generateCoordinatesXML(node)
    # We must distinguish invoiced client from delivery client
    if getattr(self, "is_delivery", None) is not None and getattr(self, "is_delivery", None):
      element = etree.SubElement(node, 'category')
      element.text = 'role/ubercart_delivery'
  ...
  def _asXML(self):
    ...
    if getter_line_method is not MARKER:
      ...
      for line in getter_line_method(**parameter_kw):
        ...
        movement_dict['price'] = float(movement_dict['gross_price'])
    ...
    # Add Discount
    if self.discount_price != 0 and self.discount_price != '':
      discount_gid = b16decode(erp5_sync.getGidFromObject(integration_site.getSourceCarrierValue()))
      discount_dict =  {'price': self.discount_price,
                        'quantity' : -1,
                        'title' : 'discount on %s / %s' % (self.discount_title, self.discount_code),
                        'reference' : 'discount on %s / %s' % (self.discount_title, self.discount_code),
                        'resource' : discount_gid,
      ...
      movement_list.append(discount_dict)
    # Add delivery
    if self.delivery_price != 0 and self.delivery_price != '':
...
class UbercartLastId(LastIdBrain):
  __allow_access_to_unprotected_subobjects__ = 1
  ...
  def getId(self):
    ...
    return LastIdBrain.getId(self)

  def _asXML(self):
    xml = ""
    node = etree.Element('node', type="LastID")

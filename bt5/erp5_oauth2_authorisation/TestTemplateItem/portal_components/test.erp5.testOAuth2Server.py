...
class TestOAuth2(ERP5TypeTestCase):
  ...
  def beforeTearDown(self):
    ...
    for parent_value, id_list in parent_dict.iteritems():
  ...
  def _query(
    ...
    environ_dict = {
      ...
      for x, y in dict(header_dict).iteritems()
    }
    cookie_header = ';'.join(
      '%s="%s"' % (
        name,
        urllib.quote(cookie_dict['value']),
      ) for name, cookie_dict in dict(cookie_dict).iteritems()
      if cookie_dict
    )
    if cookie_header:
    ...
    return (
      ...
      response_header_dict,
      {
        name: (
          ...
          None
        ) for name, cookie_dict in cookie_dict.iteritems()
      },
      response.body,
    )
  ...
  def _authorise(
    ...
    def updateCookieDictAndJar(set_cookie_dict):
      for key, value in set_cookie_dict.iteritems():
        if value is None:
  ...
  def test_multipleLoginForm(self):
    ...
    for index, (login_state_cookie_name, query_result) in enumerate(
      login_form_response_dict.iteritems(),
    ):
      # We are being redirected to portal with a pair of OAuth2 cookies
      parsed_location, _, cookie_dict = self.assertIsRedirect(
        self._submitDialog(

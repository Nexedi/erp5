...
class TestOAuth2(ERP5TypeTestCase):
  ...
  def test_expired_tokens(self):
    ...
    with self.future_time_context(datetime.timedelta(days=45)):
      ...
      self.assertEqual(status, 200)
      # with the old expired cookies we cannot login
      self.assertIsRedirect(
        self._query(
          path=portal_path + '/index_html',
          method='GET',
    ...
    # when the auth token from the new cookies is expired, we can not login
    with self.future_time_context(datetime.timedelta(days=365)):
      self.assertIsRedirect(
        self._query(
          path=portal_path + '/index_html',
          method='GET',

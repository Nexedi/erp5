...
class TestjIOApiStyle(ERP5TypeTestCase):
  ...
  def checkBadJSONPayload(self, api_method):
    ...
    error_record = self.portal.restrictedTraverse("error_record_module/" + result[u"debug"].encode())
...
context.setTitle(data['title'].encode())
return json.dumps({
  "id": context.getRelativeUrl()
}, indent=2)
"""
...
import json
data = json.loads(text_data)
if not data[u"portal_type"].encode() == "Person":
...
import json
data = json.loads(text_data)
if not data[u"portal_type"].encode() == "Organisation":
...
""")
  ...
  def test_action_post_no_action_matches(self):
    ...
    self.assertEqual(400, result[u"status"])
    message = {u'person_creation': u"Data doesn't match Person Creation", u'organisation_creation': u"Data doesn't match Organisation Creation"}
    self.assertEqual(u"API-NO-ACTION-FOUND", result[u"name"])
    self.assertEqual(message, result[u"message"])
    error_record = self.portal.restrictedTraverse("error_record_module/" + result[u"debug"].encode())
    self.assertEqual(error_record.getTitle(), "API-NO-ACTION-FOUND")
  ...
  def test_action_post_action_not_listed(self):
    ...
    result = json.loads(self.postToApi(json_payload))
    self.assertEqual(400, result[u"status"])
    message = {u'organisation_creation': u"Data doesn't match Organisation Creation"}
    self.assertEqual(u"API-NO-ACTION-FOUND", result[u"name"])
    self.assertEqual(message, result[u"message"])
    error_record = self.portal.restrictedTraverse("error_record_module/" + result[u"debug"].encode())
    self.assertEqual(error_record.getTitle(), "API-NO-ACTION-FOUND")
...
""")
  ...
  def test_action_all_docs_no_action_matches(self):
    ...
    self.assertEqual(400, result[u"status"])
    message = {u'person_search': u"Data doesn't match Person Search", u'organisation_search': u"Data doesn't match Organisation Search"}
    self.assertEqual(u"API-NO-ACTION-FOUND", result[u"name"])
    self.assertEqual(message, result[u"message"])
    error_record = self.portal.restrictedTraverse("error_record_module/" + result[u"debug"].encode())
    self.assertEqual(error_record.getTitle(), "API-NO-ACTION-FOUND")
  ...
  def test_action_all_docs_action_not_listed(self):
    ...
    result = json.loads(self.allDocsToApi(json_payload))
    self.assertEqual(400, result[u"status"])
    message = {u'organisation_search': u"Data doesn't match Organisation Search"}
    self.assertEqual(u"API-NO-ACTION-FOUND", result[u"name"])
    self.assertEqual(message, result[u"message"])
    error_record = self.portal.restrictedTraverse("error_record_module/" + result[u"debug"].encode())
    self.assertEqual(error_record.getTitle(), "API-NO-ACTION-FOUND")

...
class StandardConfigurationMixin(TestLiveConfiguratorWorkflowMixin):
  ...
  def stepCheckConsistencyAlarm(self, sequence):
    ...
    """
    self.login()
    # clone the check consistency alarm and enable it for all modules, except business_configuration_module
    # because most configurator item do not implement checkConsistency fully - they only support
    # applying the configuration, but not comparing the current state with the expected state.
    # Fixing the configurator items is a TODO - then we can enable the full check here.
    ...
    alarm.setProperty(
        ...
        [m[1] for m in alarm.Alarm_viewConsistencyCheckConfiguration.my_module_list.get_value('items')
         if m[1] != 'business_configuration_module'
        ])
    alarm.activeSense()
    self.tic()
    self.maxDiff = None

...
class StandardConfigurationMixin(TestLiveConfiguratorWorkflowMixin):
  ...
  def stepCheckPurchaseSimulationScenario(self, sequence):
    ...
    start_date = stop_date = DateTime("2008/01/02")
    order = portal.purchase_module.newContent(
    ...
    self.assertEqual([d.tested_property for d in purchase_packing_list.getDivergenceList()], ['quantity'])
    ...
    self.portal.portal_workflow.doActionFor(purchase_packing_list, 'start_action')
    self.commit()
    self.portal.portal_workflow.doActionFor(purchase_packing_list, 'stop_action')
    self.tic()
    self.portal.portal_alarms.invoice_builder_alarm.activeSense()
    self.tic()
    purchase_invoice, = purchase_packing_list.getCausalityRelatedValueList(portal_type='Purchase Invoice Transaction')
    ...
    self.assertEqual(
        [(m.getQuantity(), m.getPrice(), m.getResourceValue()) for m in purchase_invoice.getMovementList()],
        [(7, 10, resource), (70, 0.03, vat_service),])
    ...
    self.assertEqual(
        sorted([
          (
            ...
            m.getDestinationValue(portal_type='Account'),
            m.getSourceValue(portal_type='Account'),
          ) for m in purchase_invoice.getMovementList(portal_type='Purchase Invoice Transaction Line')]),
        [(0, 2.1, vat_account, None),
         (0, 70, expense_account, None),
         (72.1, 0, self.portal.account_module.payable, None),])

    for line in purchase_invoice.getMovementList(portal_type='Purchase Invoice Transaction Line'):
      line.setQuantity(line.getQuantity() * 2)

...
from Products.ERP5Type.Utils import cartesianProduct, ensure_list
...
class TestXMLMatrix(ERP5TypeTestCase, LogInterceptor):
  ...
  def test_01_RenameCellRange(self):
    ...
    self.assertEqual(ensure_list(map(set, matrix.getCellRange(**kwd))), ensure_list(map(set, cell_range)))

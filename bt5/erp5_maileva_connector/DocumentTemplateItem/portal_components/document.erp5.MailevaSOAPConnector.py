...
class MailevaSOAPConnector(XMLObject):
  ...
  def generateRequestXML(self, recipient, sender, document, track_id, page_template='maileva_connection'):
    ...
    # Disable for now, since it ask DocumentCategory, but no one is for payslip
    src = open(os.path.join(os.path.dirname(Products.ERP5.__file__), "xsd", "MailevaPJSSchema.xsd"))
    xsd = etree.parse(src)

...
def normalize_email_bytes(email_bytes):
  # type: (bytes) -> str
  """
  Output of as_string() method (email.message.Message()) output is different
  between py2 and py3 (folded in py2, but not in py3...) so normalize the
  representation of email text: the fields of the message are written in a
  predefined order, with the `unixfrom` field removed and no line wrapping.

  ...
  if jupyter_code:
    ...
    if nodelist:
      ...
      for func_alias in environment_collector.getEnvironmentRemoveList():
        ...
        if not found:
          transaction.abort()
          # The easiest way to transform an exception in code into something that
          # will render beautifuly in Jupyter is catching them and using the 
          # getErrorMessageForException function.
          try:
            raise EnvironmentUndefineError(
              "Trying to remove non existing function/variable from environment: '%s'\n" % func_alias
            )
          except EnvironmentUndefineError as e:
            return getErrorMessageForException(self, e, notebook_context)

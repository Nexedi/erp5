...
if jedi_lock is None:
  logger.warning("Making new lock")
  jedi_lock = Products.ERP5Type.Utils.jedi_lock = RLock()
jedi_environment_local = getattr(Products.ERP5Type.Utils, 'jedi_environment_local', None)  # type: local
if jedi_environment_local is None:
  jedi_environment_local = Products.ERP5Type.Utils.jedi_environment_local = local()
  logger.warning("Making new environment local")

# logger.debug("Jedi locking with %s (%s)", jedi_lock, id(jedi_lock))


def ERP5Site_getPythonSourceCodeCompletionList(self, data, REQUEST=None):
  # type: (erp5.portal_type.ERP5Site, dict, ZPublisher.HTTPRequest.HTTPRequest) -> None
  ...
  with jedi_lock:
    if not hasattr(jedi_environment_local, 'environment'):
      jedi_environment_local.environment = jedi.create_environment(sys.executable, safe=False)
      logger.warning('Creating new environment %s', jedi_environment_local.environment)
    ...
    script = jedi.Script(
        ...
        script_name,
        sys_path=[STUBS_BASE_PATH] + [p for p in sys.path if 'setuptools' not in p],
        # explicitly provide an environment to prevent jedi from guessing a virtualenv
        # in case we run with VIRTUAL_ENV environment variable.
        environment=jedi_environment_local.environment,
    )
    ...
    if data.get('xxx_definition'):
      ...
      for definition in (script.goto_assignments() or script.goto_definitions()):
    ...
    if data.get('xxx_hover'):
      ...
      for definition in script.goto_definitions():
        ...
        logger.debug('hover: %s', completions)
  ...
  for property_name in sorted(vars(temp_class)):
    ...
    if isinstance(property_value, Constant.Getter):
      ...
      if hasattr(parent_class, property_name):
        ...
        docstring = (inspect.getdoc(parent_method) or '') + "\n\n--\n"
    ...
    for module in (
      ...
      if module == 'portal_type':
        ...
        with open(
          ...
          for ti in portal.portal_types.objectValues():
          ...
          if 0:
            ...
            for ti_for_group in portal.portal_types.objectValues():
        ...
        with open(
          ...
          for sub_module, portal_type in module_to_component_portal_type_mapping.items(
            ...
            with open(
              ...
              for component in portal.portal_components.contentValues():
                if component.getPortalType() != portal_type:
                  continue
                if component.getValidationState() not in (
                  'validated', 'modified',
                  ):
                  continue

...
class TabularDataTool(BaseTool):
  """
  Tabular Data Tool is a Tool to handle tabular data (2-dimentional table like data)
  in erp5 utilising pandas DataFrame.
  The tabular data type is currently a plain pandas DataFrame.
  It is intended to use for spread sheets (ODS/Excel) with ERP5 Reports.

  With Tabular Data Tool, we can do those things in reporting:
  - joining/grouping/sorting the tabular data
  - using vectorizing function to optimise time consuming report
  """
  id = 'portal_tabular'
  ...
  # The default parameter marker
  def _createEmptyDataFrame(self):
    return pd.DataFrame()

  security.declareProtected(Permissions.View, 'fillna')
  ...
  security.declareProtected(Permissions.View, 'getTabular')
  ...
  security.declarePrivate('_getPropertyDict')
  ...
  security.declareProtected(Permissions.View, 'getFromDocumentList')

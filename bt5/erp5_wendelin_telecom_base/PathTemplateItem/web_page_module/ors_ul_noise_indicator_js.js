...
(function (window, document, Math, rJS, RSVP, Plotly, URI, loopEventListener) {
  ...
  function getPlotData(date, connection_request) {
    ...
    data_list.push({
      ...
      type: 'scatter',
      line: {'color': '#1f77b4'},
      name: 'UL Noise Indicator',
      legendgroup: 'legendgroup0',
      hovertemplate: 'Date: %{x}<br>UL Noise Indicator: %{y}'
    });
    return data_list;
  }
  ...
  function plotFromData(data, element, label) {
    ...
    return Plotly.react(
      element,
      getPlotData(date, data.ul_noise_indicator),

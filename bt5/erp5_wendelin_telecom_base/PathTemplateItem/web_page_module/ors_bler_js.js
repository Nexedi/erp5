...
(function (window, document, Math, rJS, RSVP, Plotly, URI, loopEventListener) {
  ...
  function getPlotData(date, dl, ul) {
    var coclor_array = [
        {'color': '#1f77b4'},
        {'color': '#ff7f0e'}
      ],
    ...
    data_list.push({
      ...
      hovertemplate: 'Date: %{x}<br>BLER DL: %{y:.2f}%'
    });
    data_list.push({
      x: date,
      marker: {
      ...
      hovertemplate: 'Date: %{x}<br>BLER UL: %{y:.2f}%'
    });
    return data_list;
  }
  ...
  function plotFromData(data, element, label) {
    ...
    bler_dl = data.dl_bler.map(function (value) {
      return value * 100;
    });
    bler_ul = data.ul_bler.map(function (value) {
      return value * 100;
    });

    return Plotly.react(
      element,
      getPlotData(date, bler_dl, bler_ul),

...
(function (window, document, Math, rJS, RSVP, Plotly, URI, loopEventListener) {
  ...
  function plotFromData(data, element, label) {
    ...
    return Plotly.react(
      element,
      getPlotData(date, data.unsucessful_rrc_recon),
  ...
  function plotFromResponse(response, base_element) {
    ...
    // Add base before
    label = "Unsuccessful RRC Connection Attempts rate (all cells)";
    ...
    Object.entries(response).forEach(function (entry) {
      key = entry[0];
      data = entry[1];
      label = "Unsuccessful RRC Connection Attempts";
      plotContainer = document.createElement('div');
                      ...
                      return new RSVP.Queue().push(function () {
                        ...
                        .push(function (response) {
                          var key = el.getAttribute('data-id'),
                            label = "Unsuccessful RRC Connection" +
                              "Attempt CELL " + Math.floor(key).toString(),
                            data = response[key];

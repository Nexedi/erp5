...
(function (window, document, Math, rJS, RSVP, Plotly, URI, loopEventListener) {
  ...
  function getPlotData(date, dl, ul) {
    ...
    data_list.push({
      ...
      hovertemplate: 'Date: %{x}<br>Throughput DL: %{y}bps'
    });
    data_list.push({
      x: date,
      marker: {
      ...
      hovertemplate: 'Date: %{x}<br>Throughput UL: %{y}bps'
    });
    return data_list;
  }
  ...
  function plotFromData(data, element, label) {
    ...
    throughput_dl = data.dl_throughput;
    throughput_ul = data.ul_throughput;

    return Plotly.react(
      element,
      getPlotData(date, throughput_dl, throughput_ul),

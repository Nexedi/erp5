...
(function (window, document, Math, rJS, RSVP, Plotly, URI, loopEventListener) {
  ...
  function getPlotData(date, connection_request) {
    ...
    data_list.push({
      ...
      hovertemplate: 'Date: %{x}<br>Accessibility: %{y:.2f}%'
    });
    return data_list;
  }
  ...
  function plotFromData(data, element, label) {
    ...
    accessibility_percent = data.accessibility.map(function (value) {
      return value * 100;
    });

    return Plotly.react(
      element,
      getPlotData(date, accessibility_percent),

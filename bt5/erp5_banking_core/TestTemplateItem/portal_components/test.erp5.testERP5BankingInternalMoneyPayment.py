...
class TestERP5BankingInternalMoneyPayment(TestERP5BankingMixin):
  ...
  def afterSetUp(self):
    ...
    # Set some variables :
    self.internal_money_payment_module = self.getInternalMoneyPaymentModule()
  ...
  def stepCheckObjects(self, sequence=None, sequence_list=None, **kwd):
    ...
    # check that InternalMoneyPayment Module was created
    self.assertEqual(self.internal_money_payment_module.getPortalType(), 'Internal Money Payment Module')
    # check cash sorting module is empty
    self.assertEqual(len(self.internal_money_payment_module.objectValues()), 0)
  ...
  def stepCreateInternalMoneyPayment(self, sequence=None, sequence_list=None, **kwd):
    """
    ...
    self.internal_money_payment = self.internal_money_payment_module.newContent(
                                    id='internal_money_payment',
                                    portal_type='Internal Money Payment',
    ...
    self.assertNotEqual(self.internal_money_payment.getSourceReference(), '')
    self.assertNotEqual(self.internal_money_payment.getSourceReference(), None)
    # check we have only one cash sorting
    self.assertEqual(len(self.internal_money_payment_module.objectValues()), 1)
    # get the cash sorting document
    self.internal_money_payment = getattr(self.internal_money_payment_module, 'internal_money_payment')
    # check its portal type
    self.assertEqual(self.internal_money_payment.getPortalType(), 'Internal Money Payment')
    self.assertEqual(self.internal_money_payment.getBaobabSource(), self.bi_counter_vault.getRelativeUrl())
    # check that its destination is guichet_1
    self.assertEqual(self.internal_money_payment.getSourceTotalAssetPrice(), 20000.0)
    self.assertEqual(self.internal_money_payment.getBaobabDestination(), None)

    # the initial state must be draft
    self.assertEqual(self.internal_money_payment.getSimulationState(), 'draft')

#     # source reference must be automatically generated
#     #self.internal_money_payment.setSourceReference(self.internal_money_payment.Baobab_getUniqueReference())
#     self.assertNotEqual(self.internal_money_payment.getSourceReference(), None)
#     self.assertNotEqual(self.internal_money_payment.getSourceReference(), '')
  ...
  def stepInputCashDetails(self, sequence=None, sequence_list=None, **kwd):
    ...
    Input cash details
    """
    self.addCashLineToDelivery(self.internal_money_payment, 'line_1', 'Cash Delivery Line', self.billet_10000,
            ('emission_letter', 'cash_status', 'variation'),
            ('emission_letter/p', 'cash_status/valid') + self.variation_list[1:],
            {self.variation_list[1] : 1})
    self.assertEqual(self.internal_money_payment.line_1.getPrice(), 10000)

    self.addCashLineToDelivery(self.internal_money_payment, 'line_2', 'Cash Delivery Line', self.billet_5000,
            ('emission_letter', 'cash_status', 'variation'),
            ('emission_letter/p', 'cash_status/valid') + self.variation_list[1:],
            {self.variation_list[1] : 2})
    self.assertEqual(self.internal_money_payment.line_2.getPrice(), 5000)
  ...
  def stepDelInternalMoneyPayment(self, sequence=None, sequence_list=None, **kwd):
    """
  ...
  def test_01_ERP5BankingInternalMoneyPayment(self, quiet=QUIET, run=RUN_ALL_TEST):
    """

      ...
      return new RSVP.Queue()
        ...
        .push(function (extended_search) {
          ...
          if (new_content_action !== undefined) {
            console.log(new_content_action);
    ...
    .declareService(function () {
      ...
      function formSubmit() {
        ...
        return gadget.getDeclaredGadget("erp5_form")
          ...
          .push(function (data) {
            if (data.search) {
              // XXX Hack to get around using filter_action
              // See also searchfield for how the original query is displayed instead of the monstrosity
              var searchable_column_list = options.search_column_list,
                search_list = [];
              
              for (var k = 0, k_len = searchable_column_list.length; k < k_len; k += 1) {
                search_list.push(searchable_column_list[k][0] + ':"%' + data.search + '%"');
              }
              options.extended_search = '(' + search_list.join(' OR ') + ')';

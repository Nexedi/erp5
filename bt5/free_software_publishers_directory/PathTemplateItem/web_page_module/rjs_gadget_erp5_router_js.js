  ...
  function routeDisplay(gadget, command_options) {
    if (command_options.path) {
      if (command_options.args.page === undefined) {
        var publisher_patt = /"publisher".*$/,
          software_patt = /"software".*$/,
          success_case_patt = /"success_case".*$/,
          query = command_options.args.query,
          page;
        
        if (success_case_patt.test(query)) {
          page = "success_case";
        } else if (software_patt.test(query)) {
          page = "software";
        } else if (publisher_patt.test(query)) {
          page = "publisher";
        } // no else! page === undefined on non-matching query

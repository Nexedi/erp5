  ...
  gadget_klass
    ...
    .declareMethod('render', function () {
      ...
      return new RSVP.Queue()
        .push(function () {
          return RSVP.all([
            g.getUrlFor({command: 'display', options: {page: "free_software_publisher_directory"}}),
            g.getUrlFor({command: 'display', options: {page: "publisher_list"}}),
            g.getUrlFor({command: 'display', options: {page: "software_list"}}),
            g.getUrlFor({command: 'display', options: {page: "success_story_list"}})
          ]);
        })
        .push(function (all_result) {
          ...
          tmp += panel_template_body({
            "front_href": all_result[0],
            "publisher_href": all_result[1],
            "software_href": all_result[2],
            "success_story_href": all_result[3]
          });
          return tmp;
        })

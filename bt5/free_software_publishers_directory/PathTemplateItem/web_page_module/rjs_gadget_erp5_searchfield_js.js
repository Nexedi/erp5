  ...
  gadget_klass
    ...
    .declareMethod('render', function (option_dict) {
      ...
      return new RSVP.Queue()
        .push(function () {
          function getQuery(extended_search) {
            if (!extended_search) return undefined;
            var query,
              re = /%[a-z]*\-*\ *[a-z]*%/i,
              match = extended_search.match(re);
            if (match) {
              query = match[0].slice(1, match[0].length - 1); 
            } else {
              query = extended_search;
            }
            return query;
          }
          
          var query = getQuery(option_dict.extended_search);
          return search_gadget.translateHtml(panel_template({
            widget_value: query || placeholder,

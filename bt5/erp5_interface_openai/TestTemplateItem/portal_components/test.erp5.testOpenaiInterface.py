...
class TestOpenaiConnector(ERP5TypeTestCase):
  ...
  def getBusinessTemplateList(self):
    return ('erp5_base', 'erp5_web_service', 'erp5_interface_openai')

  def afterSetUp(self):
    self.connector_reference = "test_openai_connector"
    self.base_url = "https://mock.example.com/v1"
    self._dropExistingConnectorList()

  def beforeTearDown(self):
    self.abort()
    self._dropExistingConnectorList()

  def _dropExistingConnectorList(self):
  ...
  def test_getModelList(self):
    connector = self._createConnector()

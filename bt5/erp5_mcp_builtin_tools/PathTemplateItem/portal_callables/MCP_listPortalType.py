...
# FIXME In some corner cases, count may be wrong - this is the case if a user
#   defined a custom list method that auto-maps columns to other columns or
#   does do any other unexpected 'magic'. In this case the query result may
#   be smaller than what count suggests.
count = portal.portal_catalog.countResults(
  portal_type=portal_type_id, ignore_unknown_columns=True, **kwargs
)[0][0]
start = page * page_size
end = min(start + page_size, count)

row_list = []
brains = context.PortalType_getListMethod(

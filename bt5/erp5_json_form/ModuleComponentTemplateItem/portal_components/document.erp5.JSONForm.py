...
class JSONForm(JSONType, TextDocument):
  ...
  def __call__(self, text_content="", list_error=False):
    validation_result = self.validateJsonSchema(text_content, list_error)
    ...
    if self.getAfterMethodId():
      return getattr(getattr(self, 'aq_parent', None), self.getAfterMethodId())(text_content, self.getResponseSchema())
    return "Nothing to do"

  def validateJsonSchema(self, text_content, list_error=False):
    """
    ...
    json_input = json.loads(text_content)
    try:
      jsonschema.validate(json_input, defined_schema, format_checker=jsonschema.FormatChecker())
      ...
      if list_error:
        ...
        return {
          defined_schema["$id"].decode(): [
            ("Validation Error", x.message) for x in sorted(validator.iter_errors(json_input), key=lambda e: e.path)
          ]
        }
      return err
    return True

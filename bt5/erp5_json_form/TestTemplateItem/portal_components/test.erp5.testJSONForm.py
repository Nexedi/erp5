...
"""
  ...
  def test_call_malformated_json(self):
    """
    """
    schema = """{
	"$schema": "https://json-schema.org/draft/2019-09/schema",
  "$id": "my-schema.json",
    ...
    self.fixJSONForm(method, schema, after_method)
    self.tic()
    self.assertRaises(ValueError, getattr(self.portal, method), json_data)
  ...
  "$id": "my-schema.json",
    ...
    self.fixJSONForm(method, schema, after_method)
    self.tic()
    self.assertRaises(jsonschema.exceptions.ValidationError, getattr(self.portal, method), json_data)
  ...
  "$id": "my-schema.json",
    ...
    error = {"my-schema.json": [["Validation Error", "u'2' is not of type u'integer'"], ["Validation Error", "2 is not of type u'string'"]]}
    try:
      getattr(self.portal, method)(json_data, list_error=True)
      raise ValueError("No error raised during processing")
    except ValueError, e:
      self.assertEqual(json.loads(str(e)), error)
  ...
  "$id": "my-schema.json",
    ...
    error = {
      "my-schema.json": [[
        "Validation Error",  u"u'2018-11-13T20:20:67' is not a u'date-time'"
      ]]
    }
    try:
      getattr(self.portal, method)(json_data, list_error=True)

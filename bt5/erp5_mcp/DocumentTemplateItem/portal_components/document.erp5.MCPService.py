...
try:  # duplication wrt neo/client/patch
  from ZODB.Connection import TransactionMetaData  # pylint: disable=no-name-in-module
...
try:
  from erp5.component.module.JsonUtils import loadJson  # pylint: disable=no-name-in-module
except ImportError:  # loadJson is merely cosmetics for better repr
...
class MCPService(XMLObject):
  ...
  # TODO By default DELETE must be allowed for authenticated users
  ...
  def DELETE(self, REQUEST, RESPONSE):  # == _handleDelete (WebDav)
    ...
    # FIXME Users should only be allowed to delete their own sessions!
  ...
  def _processJsonRpcMessage(self, message, request):
    ...
    try:
      ...
      if not isinstance(e, JsonRpcError):
        ...
        log("MCPService/%s: Unexpected Error: %s" % (msg_type, traceback.format_exc()))
  ...
  def __processMCPMessage(self, message, request, msg_type, validator, method_name, params):
    ...
    #   3. implement locking logic in zope (for instance through ZODB, giving
    #      MCPSession a 'is_busy' attribute).
    #
    # Option 2. seems to be the best, since it provides higher performance,
    # while avoiding ZODB transaction overhead.
    #
  ...
  def _getSession(self, request):
    ...
    try:
      session = self._session_tree[id_]
    except KeyError:
      raise JsonRpcInvalidRequestError("invalid MCP session id '%s'" % id_)
    return session

  def _delSession(self, session):
    del self._session_tree[session.id]
    # GC session to avoid orphans
    conn = self._p_jar
    storage = conn.db().storage
    txn = TransactionMetaData(
      description='MCPService._delSession(0x%x)' % u64(session._p_oid)
    )
    try:
      storage.tpc_begin(txn)
      storage.deleteObject(session._p_oid, session._p_serial, txn)
      session._p_deactivate()
  ...
  def tools_list(self, session, _meta=None, cursor=None):
    ...
    for line in self.getToolList():
      ...
      if output_schema:
        # TODO output schema must already be type dict, not a string!
        tool["outputSchema"] = json.loads(output_schema)
  ...
  def tools_call(self, session, name, _meta=None, arguments=None, task=None):
    ...
    try:
      ...
      if isinstance(e, JsonRpcError):
        raise
      log("MCPService/tools/call/%s: Unexpected error: %s" % (name, traceback.format_exc()))

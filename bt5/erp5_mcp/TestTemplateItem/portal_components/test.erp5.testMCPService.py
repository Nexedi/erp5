...
from erp5.component.module.JsonRpc import (
  JsonRpcType, JsonRpcInternalError
)
...
class MCPServiceApiTestMixin:
  ...
  def _test_tools_list(self, conn):
    ...
    info_list = _fetch_result(response)["tools"]
    ...
    for t in self.tool_list:
      ...
      if t.getOutputSchema():
        # TODO output schema must already be type dict
        self.assertEqual(i['outputSchema'], json.loads(t.getOutputSchema()))
      self.assertEqual(i['annotations']['readOnlyHint'], t.getReadOnly())

  def test_tools_call(self):
    with self.connection() as conn:
      self._initialize(conn)
      self._tools_call_ok(conn)
      # must work when skipping optional arguments
      self._tools_call_ok(conn, {"reference": "mcptest_test_person2"})
      self._tools_call_ok_structured(conn)
      self._tools_call_err_tool_runtime(conn)
      self._tools_call_err_missing_required_field(conn)
      self._tools_call_err_bad_parameter_type(conn)
      self._tools_call_err_non_existent_tool(conn)
      self._tools_call_err_missing_session_id(conn)

  def _tools_call_ok(self, conn, param_dict=None):
  ...
  def _tools_call_ok_structured(self, conn):
  ...
  def _tools_call_err_tool_runtime(self, conn):
    """Test error in tool code itself"""
    param_dict = self._test_person_data
    result = self._tools_call(conn, "mcptest_createPerson", param_dict)
    m = 'Could not create person: The id "%s" is invalid - it is already in use.' % param_dict["reference"]
    self.assertToolError(result, equals=m)

  def _tools_call_err_missing_required_field(self, conn):
  ...
  def _tools_call_err_bad_parameter_type(self, conn):
    param_dict = dict(reference="t", first_name=["bad_type"])
    result = self._tools_call(conn, "mcptest_createPerson", param_dict)
    m = ["input arguments don't match schema", "is not of type 'string'"]
    self.assertToolError(result, contains=m)

  def _tools_call_err_non_existent_tool(self, conn):
  ...
  def _tools_call_err_missing_session_id(self, conn):
    ...
    data = "messages must set 'MCP-Session-Id' in their header"
    self.assertRpcError(response, -32600, "Invalid Request", data)
  ...
  def test_invalid_session_id(self):
    with self.connection() as conn:
      id_ = conn.session_id = 2**128
      response = conn.tools_list()
      data = "invalid MCP session id '%s'" % id_
      self.assertRpcError(response, -32600, "Invalid Request", data)

  def test_concurrent_sessions(self):
    with self.connection() as conn1:
      self._initialize(conn1)

      with self.connection() as conn2:
        self._initialize(conn2)
  ...
  def test_not_validated_mcp_service(self):
    ...
    with self.connection(draft_mcp_service.absolute_url_path()) as conn:
  ...
  def test_tools_list_bad_config(self):
    ...
    self.tic()
    with self.connection(mcp_service.absolute_url_path()) as conn:
      self._initialize(conn)
      response = conn.tools_list()
...
class MCPServiceTestCase(ERP5TypeTestCase):
  ...
  def newTool(self, *args, **kw):
    ...
    input_parameter_list = kw.pop('input_parameter_list', [])
    ...
    for i, info in enumerate(input_parameter_list):
      param = self.newContent(
        tool,
      ...
      if default is not None:
        param.setParameterDefault(default)
  ...
  @contextmanager
  def connection(self, server_url=None):
    conn = MCPClientConnection(self, server_url)
    try:
      yield conn
    finally:
      conn.close()
  ...
  def _tools_call(self, conn, name, arguments):
    r = _fetch_result(conn.tools_call(name=name, arguments=arguments))
    self.tic()
    return r
...
# Helper to simplify ad-hoc tool creation
_ParameterInfo = namedtuple('ParameterInfo', ['reference', 'type_', 'default'])
class ParameterInfo(_ParameterInfo):
  __slots__ = ()  # Prevent __dict__ creation

  def __new__(cls, reference, type_, default=None):
    return _ParameterInfo.__new__(cls, reference, type_, default)
...
TOOL_CREATE_PERSON = dict(
  ...
  input_parameter_list=[
    ...
    for _ in [
      ["reference", "str"],
      ["first_name", "str", "python: None"],
      ["last_name", "str", "python: None"]
    ]
  ],
...
class MCPClientConnection(object):
  def __init__(self, test, server_url=None):
    self.test = test
    self.server_url = server_url or self.test.mcp_service.absolute_url_path()
  ...
  def close(self):
    ...
    self.test.assertIn(self.session_id, self.test.mcp_service._session_tree)
    session = self.test.mcp_service._session_tree[self.session_id]
    self.test.assertEqual(session.id, get(session).id)
    response = self.test.publish(
      self.test.mcp_service.absolute_url_path(),
    ...
    self.test.tic()
    self.test.assertNotIn(self.session_id, self.test.mcp_service._session_tree)
    self.test.assertRaises(POSKeyError, get, session)
    self.session_id = None
...
def _fetch_result(response):
  return json.loads(response.getBody())["result"]

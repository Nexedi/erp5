...
(function (window, rJS, RSVP, URI, loopEventListener, promiseEventListener, document,
  ...
  function listenToInput(gadget, index) {
    ...
    function setSelectedElement(event) {
      var element = event.target,
        ...
        tmp;
      ul.innerHTML = "";
      if (jump_url || create_object_type) {
      ...
      props.jump_url[index] = field_json.relation_item_relative_url[index];
      plane.href = gadget.props.input_list[index].href;
      input.value = gadget.props.input_list[index].value;
      if (plane.href) {
        plane.className = jump_on;
      } else {
        plane.className = jump_off;
        ...
        return new RSVP.Queue()
          ...
          .push(function (event) {
            ...
            if (ul.innerHTML) {
              ...
              input.value = gadget.props.input_list[index].value;
              props.jump_url[index] = field_json.relation_item_relative_url[index];
              if (gadget.props.input_list[index].href) {
                plane.className = jump_on;
              } else {
                plane.className = jump_off;
              }
  ...
  rJS(window)
    ...
    .declareMethod('render', function (options) {
      ...
      tmp =field_json.key.split("_");
      access_flag = tmp[tmp.lenght - 2];
      return new RSVP.Queue()
        .push(function () {

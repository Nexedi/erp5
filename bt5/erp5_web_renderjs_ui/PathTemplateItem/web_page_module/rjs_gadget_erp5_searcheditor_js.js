...
(function (window, document, rJS, Handlebars, RSVP,
  ...
  function updateFilterDOM(gadget, auto_remove, immediate_remove) {
    var item_list = gadget.element.querySelectorAll(".filter_item"),
      input_list = null,
      input_to_remove_list = [],
      last_input = null,
      i = 0,
      j = 0;
        ...
        if (auto_remove) {
          ...
          for (j = input_list.length - 2; j >= 0; j -= 1) {
            if (!input_list[j].value) {
              input_to_remove_list.push(input_list[j]);
            }
          }
        }
        input_list = item_list[i].querySelectorAll("input");
    ...
    gadget.removeInputList(input_to_remove_list, immediate_remove);
  }
  ...
  gadget_klass
    ...
    .declareJob('removeInputList', function (input_list, immediate_remove) {
      function doRemove() {
        var i = 0;
    ...
    .onEvent('submit', function submit() {
      updateFilterDOM(this, true, true);

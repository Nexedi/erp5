...
(function (window, document, rJS, Handlebars, RSVP,
      ...
      if ("at_least_one_exact_match" ===
        ...
        for (j = input_list.length - 2; j >= 0; j -= 1) {
          if (!input_list[j].value &&
              input_list[j] !== gadget.state.focused_element) {
            input_list[j].remove();
          }
        }
  ...
  gadget_klass
    ...
    .declareJob('updateFilterDOM', function updateFilterDOM(options) {
      var gadget = this,
        p = new RSVP.Queue(),
      ...
      return p.push(function () {
        return gadget.changeState({update_filter_dom: gadget.state.update_filter_dom + 1});
      });
    })
    ...
    .onEvent('blur', function blur() {
      this.state.focused_element = null;
    }, true, false)

    .onEvent('focus', function focus(evt) {
      this.state.focused_element = evt.target;
    }, true, false)

    .onEvent('change', function change(evt) {
      this.updateFilterDOM({delay: 4});  // let time for blur/focus event to act
...
}(window, document, rJS, Handlebars, RSVP,
  QueryFactory, SimpleQuery, ComplexQuery, Query));
...
(function (window, document, rJS, Handlebars,
  ...
  function detectAtleastoneexactmatchComplexQuery(query) {
    ...
    if (query.type !== "complex") { return null; }
    ...
    return [key, operator, value_list];
  }
  ...
  function createFilterItemTemplate(gadget, query_dict) {
    ...
    if (query_dict.type === "complex") {
      query_details = detectAtleastoneexactmatchComplexQuery(query_dict);
      operator_default_list = OTHER;

      is_selected = false;
      for (i = 0; i < gadget.state.search_column_list.length; i += 1) {
        is_selected = is_selected || (query_details[0] === gadget.state.search_column_list[i][0]);
        column_option_list.push({
          text: gadget.state.search_column_list[i][1],
          value: gadget.state.search_column_list[i][0],
          selected: (query_details[0] === gadget.state.search_column_list[i][0])
        });
      }
      if (!is_selected) {
        throw new Error('SearchEditor: no key found for: ' + query_details[0]);
      }
      is_selected = false;
    ...
    .declareMethod('render', function render(options) {
      ...
      if (options.extended_search) {
        ...
        if (jio_query instanceof SimpleQuery) {
          ...
          if (sub_jio_query_details) {
            querySetKeyInDeep(sub_jio_query_dict, PREFIX_COLUMN + sub_jio_query_details[0]);
            query_list.push(sub_jio_query_dict);
            operator = "AND";
            jio_query = {query_list: []};  // XXX this line acts like a "go to end of this function"
          }

...
(function (window, rJS, RSVP, URI, loopEventListener, promiseEventListener,
  ...
  rJS(window)
    ...
    .declareMethod('render', function (options) {
      ...
      gadget.props.jump_url = field_json.relation_item_relative_url;
      ...
      if (!value && target_url && field_json.uid) {
        //return from listbox
        queue
          .push(function () {
      ...
      queue
        .push(function (href) {
          gadget.props.original_href = href;
          gadget.props.original_value = value;
    ...
    .declareMethod('getContent', function () {
      ...
      if (this.props.plane.className === jump_add) {
        ...
        result[element.getAttribute('name')] = this.props.new_value;
      } else {
    ...
    .declareMethod('getNonSavedValue', function () {
      var element = this.props.element.querySelector('input'),
        result = {},
        field_json = this.props.field_json;
      delete field_json.create_object;
      if (this.props.plane.className === jump_add) {
        field_json.create_object = this.props.create_object_type;
        field_json.default = [this.props.new_value];
        field_json.relation_item_relative_url = [];
      } else {
        field_json.default = [element.value];
    ...
    .declareService(function () {
      ...
      function setSelectedElement(event) {
        ...
        if (create_object_type) {
          gadget.props.new_value = props.input.value;
          ...
          return new RSVP.Queue()
            ...
            .push(function (stored_data) {
              return gadget.relation_jump({
                command: 'index',
                options: {
                  jio_key: gadget.props.field_json.url,
        ...
        props.plane.href = gadget.props.original_href;
        gadget.props.input.value = gadget.props.original_value;
        if (gadget.props.original_href) {
          props.plane.className = jump_on;
        } else {
          props.plane.className = jump_off;
        }

...
if context.WebSite_getTranslationDataTextContent(
  ...
  if fixit:
    ...
    already_fixed[gadget_translation_data_js_reference].add(context.getId())
    if len(already_fixed[gadget_translation_data_js_reference]) > 1:
      raise RuntimeError(
          ...
          % (
              ...
              ", ".join(already_fixed[gadget_translation_data_js_reference]),
          ))

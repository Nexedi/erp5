...
class TestListField(ERP5TypeTestCase):
  ...
  def test_render(self):
    select, input, = lxml.html.fragments_fromstring(self.field.render())
    # listfields render an input to confirm that the field was posted
    # in the form's action script
    self.assertEqual(input.name, 'default_field_test_field:int')
    self.assertEqual(input.type, 'hidden')
  ...
  def test_render_escape_html(self):
    ...
    (script, ), _, = lxml.html.fragments_fromstring(self.field.render())
  ...
  def test_render_disabled(self):
    ...
    (disabled, ), _, = lxml.html.fragments_fromstring(self.field.render())

...
class TestERP5Core(ERP5TypeTestCase, ZopeTestCase.Functional):
  ...
  def check_actions(self, target, expected):
    ...
    got = {}
    for category, actions in list(actions.items()):
  ...
  def test_Folder_delete_related_object(self):
    ...
    def delete(assert_deleted, obj):
      ...
      self.assertTrue((urllib.parse.quote('Sorry, 1 item is in use.'), 'Deleted.')[assert_deleted]
  ...
  def test_Folder_delete_non_accessible_object(self):
    ...
    self.assert_(urllib.parse.quote('Sorry, 1 item is in use.') in redirect, redirect)
  ...
  def test_BasicAuthenticateDesactivated(self):
    ...
    _, api_netloc, _, _, _ = urllib.parse.urlsplit(self.portal.absolute_url())

    connection = http.client.HTTPConnection(api_netloc)
    connection.request(
      method='GET',

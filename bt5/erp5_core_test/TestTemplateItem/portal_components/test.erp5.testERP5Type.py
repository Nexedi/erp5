...
class PropertySheetTestCase(ERP5TypeTestCase):
  ...
  def _addProperty(self, portal_type_name, property_sheet_id,
    ...
    if property_id is not None:
      ...
      ps.newContent(reference=property_id, **kw)
    if commit:
      self.commit()
...
class TestERP5Type(PropertySheetTestCase, LogInterceptor):
  ...
  def test_03_NewTempObject(self):
    ...
    class WriteError(Exception):
      pass
    def _setLastId(self, id): # pylint: disable=redefined-builtin
      raise WriteError
...
#       foo.setDefaultRegionValue(None)
  ...
  def test_translated_category_accessors(self):
    ...
    gamma = region_category.newContent(portal_type="Category",
                                       id="gamma",
                                       title="Gamma System")
    gamma.newContent(portal_type='Category', id='alpha', title='Alpha')
    self.portal.Localizer = DummyLocalizer()
    doc = self.portal.person_module.newContent(portal_type='Person',
                                               region='gamma/alpha')
  ...
  def test_renameObjectsReindexSubobjects(self):
    ...
    final_id = 'bar'
    subdocument_id = 'sub'
    obj = folder.newContent(portal_type='Organisation', id=initial_id)
    obj.newContent(id=subdocument_id)
    self.tic()
    folder = self.getOrganisationModule()
  ...
  def test_copyWithoutModificationRight(self):
    ...
    script_id = '%s_afterClone' % (obj.getPortalType().replace(' ', ''), )
    createZODBPythonScript(script_container, script_id, '', 'context.setTitle("couscous")')
    try:
      security_manager = getSecurityManager()
      self.assertEqual(1, security_manager.checkPermission('Access contents information', obj))
      self.assertEqual(1, security_manager.checkPermission('Modify portal content', obj))
      obj.manage_permission('Modify portal content')
      clipboard = folder.manage_copyObjects(ids=[obj.id])

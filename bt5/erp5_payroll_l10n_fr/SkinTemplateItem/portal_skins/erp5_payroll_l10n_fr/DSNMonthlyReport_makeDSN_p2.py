...
getEmployeeRemunerationAsList = context.DSNMonthlyReport_getEmployeeRemunerationAsList
...
# category_list = context.category_list.data.split('\n')
# rubric_dict = {}

# for line_number, line in enumerate(category_list):
#   if line_number==0:
#     pass
#   else:
#     data = line.split(',')
...
paysheet_id_list = sorted(context.getAggregateRelatedIdList()) # Sorting for idempotent result in tests
organisation = context.getSourceSectionValue()
organisation_contact = context.getSourceAdministrationValue()
establishment = portal.accounting_module[paysheet_id_list[0]].getDestinationSectionValue()

leave_period_dict = context.DSNMonthlyReport_getLeavePeriodAsDict()
employee_list = []
...
# DSN HEADERS
dsn_file.append(getDSNBlockAsDict(block_id='S10.G00.00'))
dsn_file.append(getDSNBlockAsDict(block_id='S10.G00.01', target=organisation))
dsn_file.append(getDSNBlockAsDict(block_id='S10.G00.02', target=organisation_contact))
dsn_file.append(getDSNBlockAsDict(block_id='S10.G00.03', target=organisation))

# Monthly DSN
dsn_file.append(getDSNBlockAsDict(block_id='S20.G00.05', year=declared_year, month=declared_month, order=dsn_order))
dsn_file.append(getDSNBlockAsDict(block_id='S21.G00.06', target=organisation))

dsn_file.append(getDSNBlockAsDict(block_id='S21.G00.11', target=establishment, manpower=len(paysheet_id_list)))
dsn_file.append(getDSNBlockAsDict(block_id='S21.G00.15', target=establishment))

# Print aggregated cotisations
cotisation_dict = context.DSNMonthlyReport_getSocialContributionAsDict(paysheet_id_list)

bloc_versement = getDSNBlockAsDict(block_id='S21.G00.20', target=establishment, year=declared_year, month=declared_month)
for rubric, value in cotisation_dict['payment']:
  bloc_versement[rubric] = value
dsn_file.append(bloc_versement)

bloc_payment_slip = getDSNBlockAsDict(block_id='S21.G00.22', target=establishment, year=declared_year, month=declared_month)
...
change_block_dict = context.DSNMonthlyReport_getChangeBlockAsDict()
...
for paysheet_id in paysheet_id_list:
  ...
  # Get the corresponding enrollment item
  career_step_list = employee.searchFolder(portal_type="Career",
                                           subordination_uid=establishment.getUid())
  for career_step in career_step_list:
    if career_step.getValidationState() == "open":
      break
  else:
    raise AttributeError("No Payroll Enrollment Record found for employee %s" % employee.getRelativeUrl())

  item = career_step.getAggregateValue()

  dsn_file.append(getDSNBlockAsDict(block_id='S21.G00.30', target=employee, enrollment_item=item))

  for date in sorted(employee_change_block_dict.get('S21.G00.31', ())):
    dsn_file.append(getDSNBlockAsDict(block_id='S21.G00.31', change_bloc=employee_change_block_dict['S21.G00.31'][date], change_date=date))

  dsn_file.append(getDSNBlockAsDict(block_id='S21.G00.40', target=employee.getDefaultCareerValue(), enrollment_item=item))

  for date in sorted(employee_change_block_dict.get('S21.G00.41', ())):
    dsn_file.append(getDSNBlockAsDict(block_id='S21.G00.41', change_bloc=employee_change_block_dict['S21.G00.41'][date], change_date=date))
  ...
  last_date_of_month = LastDateOfMonth(context.getEffectiveDate())
  first_date_of_month = DateTime(context.getEffectiveDate().year(),
                                 context.getEffectiveDate().month(),
                                 1)
  
  if item.getCareerStopDate() != None and \
     first_date_of_month <= item.getCareerStopDate() <= last_date_of_month:
    dsn_file.append(getDSNBlockAsDict("S21.G00.62", enrollment_item=item))

  dsn_file.append(getDSNBlockAsDict(block_id='S21.G00.70', target=employee, enrollment_item=item))
  
  for date in sorted(employee_change_block_dict.get('S21.G00.72', ())):
    dsn_file.append(getDSNBlockAsDict(block_id='S21.G00.72', change_bloc=employee_change_block_dict['S21.G00.72'][date], change_date=date))
  
  dsn_file.append(getDSNBlockAsDict(block_id='S21.G00.71', enrollment_item=item))
  
  dsn_file.append(getDSNBlockAsDict(block_id='S21.G00.50', target=paysheet))

  for remuneration in getEmployeeRemunerationAsList(paysheet_id=paysheet_id, enrollment_item=item):
  ...
  for base_code in sorted(individual_cotisation_dict):
    if float(individual_cotisation_dict[base_code]) != 0.:
      dsn_file.append(getDSNBlockAsDict(block_id='S21.G00.78',
                                      target=paysheet,
                                      base_code=base_code,
  ...
  if cice_relative_min_salary:
    dsn_file.append(getDSNBlockAsDict(block_id='S21.G00.79',
                                     base_code='02',
  ...
  if fillon_relative_min_salary:
    ...
    dsn_file.append(getDSNBlockAsDict(block_id='S21.G00.81',
                                     base_code='018',
  ...
  for employee in leave_period_dict:
    for period in leave_period_dict[employee]:
      ...
      if employee in employee_list:
        dsn_order += 1
        dsn_file.append(getEventDSNBlockAsDict(block_id='S20.G00.05', dsn_type='04', order=dsn_order)) #'04' is DSN Leave Event
        dsn_file.append(getEventDSNBlockAsDict(block_id='S20.G00.07', target=organisation_contact))
        dsn_file.append(getEventDSNBlockAsDict(block_id='S21.G00.06', target=organisation))
        dsn_file.append(getEventDSNBlockAsDict(block_id='S21.G00.11', target=employee.getDefaultCareerValue().getSubordinationValue()))
        dsn_file.append(getEventDSNBlockAsDict(block_id='S21.G00.30', target=employee))
        dsn_file.append(getEventDSNBlockAsDict(block_id='S21.G00.40', target=employee.getDefaultCareerValue()))
...
# Footer block
footer = getDSNBlockAsDict(block_id='S90.G00.90', length=rubric_counter, dsn_record_counter=dsn_order)

...
def getBaseFromCTP(ctp):
  #XXX : For ctp 400D, should return sum since beginning of the year
  ctp_code = ctp[:3]
  ctp_category = ctp[-1]
  if ctp_category == 'P':
    result = fee_per_ctp_dict[ctp_code]['capped']
  elif ctp_category == 'D':
    result = fee_per_ctp_dict[ctp_code]['uncapped']
  else:
    result = fee_per_ctp_dict[ctp_code]['other']
  return result


def updateIndividualFeeDict(fee_dict, code, category, base, rate=0.0):
  "Sums a base to the correct CTP, and update the rate if needed"
  if code not in fee_dict:
    fee_dict[code] = 0.
  if category == 'P':
    fee_dict[code]['capped'] = fee_dict[code]['capped'] + float(base)
  elif category == 'D':
    fee_dict[code]['uncapped'] = fee_dict[code]['uncapped'] + float(base)
  else:
    fee_dict[code]['other'] = fee_dict[code]['other'] + float(base)
...
payroll_enrollment_enrollment_record_search = portal.payroll_enrollment_record_module.searchFolder
...
for paysheet_id in paysheet_list:
  ...
  # Get the corresponding enrollment record
  career_step_list = employee.searchFolder(portal_type="Career",
                                           subordination_uid=paysheet.getDestinationSectionValue().getUid())
  ...
  enrollment_record = career_step.getAggregateValue()

...
class _ERP5(ERP5TypeTestSuite):
  ...
  def _getAllTestList(self):
    ...
    component_re = re.compile(".*/([^/]+)/TestTemplateItem/portal_components"
                              "/test\.[^.]+\.([^.]+).py$")
    for test_path in (
        glob('%s/bt5/erp5_hal_json_style/TestTemplateItem/portal_components/test*.py' % path) +
        glob('%s/bt5/erp5_accounting_renderjs_ui_test/TestTemplateItem/portal_components/test*.py' % path) +

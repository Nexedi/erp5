...
class TestERP5BankingCashIncident(TestERP5BankingMixin, ERP5TypeTestCase):
  """
  ...
  def getCashIncidentModule(self):
    ...
    """
    return getattr(self.getPortal(), 'cash_incident_module', None)
  ...
  def afterSetUp(self):
    ...
    self.cash_incident_module = self.getCashIncidentModule()
  ...
  def stepCheckObjects(self, sequence=None, sequence_list=None, **kwd):
    ...
    # check that CashIncident Module was created
    self.assertEqual(self.cash_incident_module.getPortalType(), 'Cash Incident Module')
    # check cash transfer module is empty
    self.assertEqual(len(self.cash_incident_module.objectValues()), 0)
  ...
  def stepCreateCashIncident(self, sequence=None, sequence_list=None, **kwd):
    ...
    self.assertEqual(len(self.cash_incident_module.objectValues()), 1)
    # get the cash transfer document
    self.cash_incident = getattr(self.cash_incident_module, 'cash_incident_1')
    # check its portal type
    self.assertEqual(self.cash_incident.getPortalType(), 'Cash Incident')
    # check that its destination is counter
    self.assertEqual(self.cash_incident.getSource(), self.counter.getRelativeUrl())
    self.assertEqual(self.cash_incident.getDestination(), None)
  ...
  def stepCreateIncomingLine(self, sequence=None, sequence_list=None, **kwd):
    ...
    self.addCashLineToDelivery(self.cash_incident, 'valid_line_1', 'Incoming Cash Incident Line', self.billet_10000,
            ('emission_letter', 'cash_status', 'variation'), ('emission_letter/p', 'cash_status/valid') + self.variation_list,
            self.quantity_10000)
    # execute tic
    self.stepTic()
    # check there is only one line created
    self.assertEqual(len(self.cash_incident.objectValues()), 1)
    # get the cash transfer line
    self.valid_line_1 = getattr(self.cash_incident, 'valid_line_1')
    # check its portal type
    self.assertEqual(self.valid_line_1.getPortalType(), 'Incoming Cash Incident Line')
    ...
    for variation in self.variation_list:
      ...
      self.assertEqual(cell.getBaobabDestination(), self.counter.getRelativeUrl())
  ...
  def stepCheckSubTotal(self, sequence=None, sequence_list=None, **kwd):
    ...
    self.assertEqual(self.cash_incident.getTotalQuantity(), 5.0)
  ...
  def stepCreateOutgoingLine(self, sequence=None, sequence_list=None, **kwd):
    ...
    """
    # create the line
    self.addCashLineToDelivery(self.cash_incident, 'valid_line_2', 'Outgoing Cash Incident Line', self.piece_200,
            ('emission_letter', 'cash_status', 'variation'), ('emission_letter/p', 'cash_status/valid') + self.variation_list,
            self.quantity_200)
    # execute tic
    self.stepTic()
    ...
    # check portal types
    self.assertEqual(self.valid_line_2.getPortalType(), 'Outgoing Cash Incident Line')
    ...
    for variation in self.variation_list:
      ...
      self.assertEqual(cell.getPortalType(), 'Cash Delivery Cell')
      # check the source vault is usual_cash
      self.assertEqual(cell.getBaobabSource(), self.counter.getRelativeUrl())
      # check the destination vault is counter
  ...
  def stepTryConfirmCashIncidentWithTwoDifferentLines(self, sequence=None, sequence_list=None, **kwd):
    """
    """
    # fix amount (10000 * 5.0 + 200 * 12.0 + 5000 * 24)
    self.cash_incident.setSourceTotalAssetPrice('172400.0')
    # try to do the workflow action "confirm_action', cath the exception ValidationFailed raised by workflow transition 
    self.assertRaises(ValidationFailed, self.workflow_tool.doActionFor, self.cash_incident, 'confirm_action', wf_id='cash_incident_workflow')
    # execute tic
    self.stepTic()
    # get state of the cash transfer
    state = self.cash_incident.getSimulationState()
  ...
  def stepDelOutgoingLine(self, sequence=None, sequence_list=None, **kwd):
    ...
    """
    self.cash_incident.deleteContent('valid_line_2')


  def stepTryConfirmCashIncidentWithBadPrice(self, sequence=None, sequence_list=None, **kwd):
    """
  ...
  def stepDeliverCashIncident(self, sequence=None, sequence_list=None, **kwd):
    ...
    self.workflow_tool.doActionFor(self.cash_incident, 'deliver_action', wf_id='cash_incident_workflow')
    # execute tic
    self.stepTic()
  ...
  def test_01_ERP5BankingCashIncident(self, quiet=QUIET, run=RUN_ALL_TEST):
    """
...
if __name__ == '__main__':
  ...
  def test_suite():
    ...
    suite.addTest(unittest.makeSuite(TestERP5BankingCashIncident))
    return suite

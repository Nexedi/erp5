  ...
  for wf in self.portal_workflow.objectValues():
    ...
    for transition in wf.getTransitionValueList():
      ...
      for info in info_list:
        if info.regexp.search(text) is not None:
          text = info.regexp.sub(info.new_name, text)
          line = 'Transition %s of %s is modified for %s' % (transition.getReference(), 'portal_workflow/' + wf.getId(), info.name)
          LOG('fixSkinNames', 0, line)
          msg += '%s\n' % line
          if not dry_run:
    ...
    for worklist  in wf.getWorklistValueList():
      ...
      for info in info_list:
        if info.regexp.search(text) is not None:
          text = info.regexp.sub(info.new_name, text)
          line = 'Worklist %s of %s is modified for %s' % (worklist.getReference(), 'portal_workflow/' + wf.getId(), info.name)
          LOG('fixSkinNames', 0, line)
          msg += '%s\n' % line
          if not dry_run:
    ...
    for script in wf.getScriptValueList():
      ...
      for info in info_list:
        if info.regexp.search(text) is not None:
          ...
          name_list.append(info.name)
      if len(name_list) > 0:
        line = 'Script %s of %s is modified for %s' % (script.getReference(), 'portal_workflow/' + wf.getId(), ', '.join(name_list))
        LOG('fixSkinNames', 0, line)
        msg += '%s\n' % line
        if not dry_run:
          REQUEST['BODY'] = text

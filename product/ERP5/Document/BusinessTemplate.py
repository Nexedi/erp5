...
def _redirectCatalogContainerId(portal, catalog_id):
  default_id = getattr(portal.portal_catalog, 'default_erp5_catalog_id', None)
  ...
  def build(self, context, **kw):
    ...
    for relative_url in self._archive:
      ...
      if len(url_list) > 2 and url_list[0] == 'portal_catalog':
        url_list[1] = _redirectCatalogContainerId(p, url_list[1])
      ...
      if path in update_dict or force:
        ...
        if len(container_path) > 1 and container_path[-2] == 'portal_catalog':
          container_path = container_path[:-1] + [
            _redirectCatalogContainerId(portal, container_path[-1])]
        try:
  ...
  def install(self, context, trashbin, **kw):
    ...
    if catalog.meta_type == 'ERP5 Catalog':
      ...
      for path, obj in six.iteritems(self._objects):
        ...
        # The method has been installed into the (redirected) default catalog,
        # not at the catalog id declared in the path, so resolve it there.
        url_list = path.split('/')
        if len(url_list) > 2 and url_list[0] == 'portal_catalog':
          url_list[1] = _redirectCatalogContainerId(portal, url_list[1])

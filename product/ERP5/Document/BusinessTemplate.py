...
Business Template is a set of definitions, such as skins, portal types and categories. This is used to set up a new ERP5 site very efficiently."""
    ...
    def getMigratableSourceCodeFromFilesystemList(self,
      ...
      for product_name, product_obj in inspect.getmembers(Products,
        ...
        for submodule_name, submodule_obj in inspect.getmembers(product_obj,
          ...
          if submodule_filepath.rsplit('/', 1)[0] == product_base_path:
            ...
            LOG('ModuleComponent', INFO,
                "%r: %r: %r" % (submodule_obj, submodule_name, submodule_filepath))
            ...
            # => ZODB Component Reference: ISmsSendingGateway
              ...
              if interface_class_name is None:
                LOG(component_portal_type, INFO,
                    "SKIPPED INTERFACE: %r: %r: %r" % (source_reference, subsubmodule_name, filepath))
                continue

              subsubmodule_name = interface_class_name

            LOG(component_portal_type, INFO,
                "%r: %r: %r" % (source_reference, subsubmodule_name, filepath))
    ...
    def migrateSourceCodeFromFilesystem(self,
      ...
      #
      # TODO-BEFORE-MERGE: checkPythonSource code done twice
      # (importFromFilesystem() and newContent() through Interaction Workflow)
      transaction.commit()

      # TODO-BEFORE-MERGE: Probably not needed anymore because we don't
      # validate automatically by default anymore...

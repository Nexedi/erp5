      ...
      if len(previous_test_result_list):
        ...
        for line in previous_test_result.objectValues():
          if line.getSimulationState() in ('stopped', 'public_stopped'):
            ...
            # latest to finish).
            test_priority_list.append((line.getTitle(),
                (line.getStringIndex() == 'PASSED' and 1 or 0,
                 -line.getProperty('duration'))))
      test_priority_list.sort(key=lambda x: x[1])
      sorted_test_list = [x[0] for x in test_priority_list]

...
for (portal_type, workflow), state_count_dict in sorted(iter(type_workflow_state_count_dict_dict.items()), key=lambda x: x[0]):
  if sum(state_count_dict.values()):
    ...
    for state, count in sorted(iter(state_count_dict.items()), key=lambda x: x[0]):
      if count:
        append({

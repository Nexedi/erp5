...
def getCategoryChildCompactLogicalPathItemList(base_category, **kw):
  return [i for i in base_category.getCategoryChildCompactLogicalPathItemList(base=1) if i[1] == "" or getattr(ctool.restrictedTraverse(i[1]), "getValidationState", str)() in ("", "embedded", "published", "protected")]  # [:]

for item in mixed_list:
  base_category = ctool[item]
  item_list = getCategoryChildCompactLogicalPathItemList(base_category, base=1)
  if item_list == [['', '']]:
    for fallback_category_id in base_category.getFallbackBaseCategoryList():
      ...
      if fallback_category is not None and fallback_category.objectIds():
        item_list.extend([('%s/%s' % (fallback_category_id, x[0]), '%s/%s' % (item, x[1])) \
          for x in getCategoryChildCompactLogicalPathItemList(fallback_category, base=1) if x[0]])
        break

...
for prop_dict in context.getPropertyMap():
  ...
  # If new_value and old_value are different and the property type
  # is not in data, text or iteratables, then we display the diff
  if ((new_value != old_value or error)
      and old.getPropertyType(prop) not in ('data', 'text', 'tokens')):

...
class SimulationTool(BaseTool):
  ...
  def _generateSQLKeywordDictFromKeywordDict(self, table='stock', sql_kw=None,
    ...
    if sql_kw.get('group_by_time_sequence_list'):
      all_time_sequence_have_from_date_to_date = True
      for i, time_sequence in enumerate(sql_kw['group_by_time_sequence_list']):
        has_from_date = time_sequence.get('from_date') is not None
        has_at_date = time_sequence.get('at_date') is not None
        has_to_date = time_sequence.get('to_date') is not None
        if not (has_from_date and has_to_date):
          all_time_sequence_have_from_date_to_date = False
        if not (has_from_date or has_at_date or has_to_date):
          raise ValueError("Invalid time sequence at index {}: {!r}".format(i, time_sequence))
      sql_kw['group_by_time_sequence_join'] = 'from_date_to_date' if all_time_sequence_have_from_date_to_date else None
  ...
  def getInventoryList(self, src__=0, optimisation__=True,
    ...
    if interpolation_method in ('linear', 'all_or_nothing', 'one_for_all'):
      ...
      # this optimization can not be applied with an interpolation method
      stock_sql_kw['group_by_time_sequence_join'] = None

...
class DomainTool(BaseTool):
  ...
  def generateMultivaluedMappedValue(self, context, test=1,
    ...
    if predicate_list:
      ...
      for mapped_value_property, v in six.iteritems(per_parent_mapped_value_property_dict):
        for _, mapped_value_list in sorted(six.iteritems(v), key=lambda x: parent_sort_index[x[0]]):
      ...
      mapped_value._setMappedValuePropertyList(
        list(mapped_value_property_dict))

...
class SelectionTool( BaseTool, SimpleItem ):
  ...
  def checkAll(self, list_selection_name, listbox_uid=None, REQUEST=None,
  ...
  def uncheckAll(self, list_selection_name, listbox_uid=None, REQUEST=None,
                 query_string=None, form_id=None):
    """
...
class TreeListLine:
  def __init__(self,object,is_pure_summary,depth, is_open,select_domain_dict,exception_uid_list): # pylint: disable=redefined-builtin
    self.object=object
...
def makeTreeList(here, form, root_dict, report_path, base_category,
                 ...
                 filtered_portal_types=None,sort_on = (('id', 'ASC'),)):
  """
...
for property_id in candidate_method_id_list:
  ...
  def listbox_portal_selection_wrapper(self, wrapper_property_id=property_id, *args, **kw):
    """
      Wrapper method for SelectionTool.
    """
    portal_selection = getToolByName(self, 'portal_selections')
  ...
  setattr(FolderMixIn, new_property_id, listbox_portal_selection_wrapper)
  security_property_id = '%s__roles__' % (property_id, )
  security_property = getattr(SelectionTool, security_property_id, None)
  if security_property is not None:

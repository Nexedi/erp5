...
CREATE TABLE `stock` (
  ...
  KEY `node_uid_date_resource_uid` (`node_uid`, `date`, `resource_uid`),
  KEY `section_resource_node_uid` (`section_uid`, `resource_uid`, `node_uid`, `simulation_state`),
  KEY `node_uid_mirror_node_uid_date_resource_uid` (`node_uid`, `mirror_node_uid`, `date`, `resource_uid`),
  KEY `date` (`date`)
) ENGINE=InnoDB;

...
<dtml-else>
  SUM(<dtml-var stock_table_id>.quantity <dtml-if transformed_uid> * transformation.quantity</dtml-if> * <dtml-var interpolation_ratio>) AS inventory,
  SUM(<dtml-var stock_table_id>.quantity <dtml-if transformed_uid> * transformation.quantity</dtml-if> * <dtml-var interpolation_ratio>) AS total_quantity,
  <dtml-if convert_quantity_result>
...
, <dtml-var stock_table_id>
   ...
   <dtml-if group_by_time_sequence_list>
     ...
     ON
      <dtml-if expr="group_by_time_sequence_join == 'from_date_to_date'">
        `stock`.`date` >= slot_from_date AND `stock`.`date` < slot_to_date
      <dtml-else>
      ((slot_from_date IS NOT NULL
        AND (slot_at_date IS NOT NULL
             AND GREATEST(`stock`.`date`, `stock`.`mirror_date`) >= slot_from_date
             AND LEAST(`stock`.`date`, `stock`.`mirror_date`) <= slot_at_date)
        OR ((slot_to_date IS NOT NULL
             AND GREATEST(`stock`.`date`, `stock`.`mirror_date`) >= slot_from_date
             AND LEAST(`stock`.`date`, `stock`.`mirror_date`) < slot_to_date)
            OR (GREATEST(`stock`.`date`, `stock`.`mirror_date`) >= slot_from_date
                AND slot_at_date IS NULL
                AND slot_to_date IS NULL)))
       OR (slot_from_date IS NULL
           AND ((slot_at_date IS NOT NULL
                 AND (LEAST(`stock`.`date`, `stock`.`mirror_date`) <= slot_at_date))

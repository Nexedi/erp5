...
class TestDomainTool(TestPredicateMixIn):
  ...
  def checkPredicate(self, test=None):
    ...
    def check(expected, portal_type):
      self.tic()
    ...
    check([predicate], self.portal_type_query)
    ...
    check([], self.portal_type_query) # XXX: mistake in the test ?
  ...
  def test_07_NonLeftJoinModeOfSearchPredicateList(self):
    ...
    def check(has_left_join, expected, **kw):

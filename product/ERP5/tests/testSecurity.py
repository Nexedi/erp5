...
class TestSecurityMixin:
  ...
  def _checkObjectAllMethodProtection(self, obj):
    ...
    for method_id in dir(obj):
      ...
      # Since py3.13 leading spaces are stripped from docstring so normalize __doc__
      method__doc__ = getattr(method, '__doc__', None)
      if method__doc__ is not None:
        method__doc__ = method__doc__.lstrip()
      if isinstance(method, MethodType) and \
        ...
        (# Implicitly Public method?
         ...
         # Method with security declaration but without a docstring?
         (not method.__doc__ and \
          getattr(obj, '%s__roles__' % method_id, ACCESS_PRIVATE) is ACCESS_PUBLIC)):
  ...
  def test_method_protection(self):
    ...
    if error_list:
      message = ('\nThe following %s methods have a docstring without security assertions '
                 'or are public methods without docstring.\n\t%s' \
                 % (len(error_list), '\n\t'.join(error_list)))
      self.fail(message)

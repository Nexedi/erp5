...
class TestInventoryModule(TestOrderMixin, ERP5TypeTestCase):
  ...
  def stepTestCalculateProduct(self, sequence=None, sequence_list=None, **kw):
    ...
    self.tic()
    inventory.Inventory_calculatingProductStock(batch_mode=True)
    self.assertEqual(inventory.getSimulationState(), 'calculating')
    self.tic()
    self.assertEqual(inventory.getSimulationState(), 'record')
    inventory.Inventory_calculatingProductStock(batch_mode=True)
    self.assertEqual(inventory.getSimulationState(), 'calculating')
    self.tic()
    self.assertEqual(inventory.getSimulationState(), 'record')

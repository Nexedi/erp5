...
class TestInventoryList(InventoryAPITestCase):
  ...
  def test_group_by_time_sequence(self):
    ...
    # now using from_date & to_date on all slots, this is optimized
    inventory_list = getInventoryList(
      node_uid=self.node.getUid(),
      group_by_time_sequence_list=(

        {'from_date': DateTime('2016/01/01'), 'to_date': DateTime('2016/02/01')},
        {'from_date': DateTime('2016/02/01'), 'to_date': DateTime('2016/03/01')},
      )
    )
    self.assertEqual([1, 3+5], [x.inventory for x in inventory_list])
  ...
  def test_group_by_time_sequence_invalid_inputs(self):
    ...
    with self.assertRaisesRegex(ValueError, "Invalid time sequence at index 0: {}"):
      getInventoryList(
        node_uid=self.node.getUid(),
        group_by_time_sequence_list=(
          {},
    ...
    with self.assertRaisesRegex(ValueError, "Invalid time sequence at index 1: {'foo': 'bar'}"):
      getInventoryList(
        node_uid=self.node.getUid(),
        group_by_time_sequence_list=(
          {'from_date': DateTime()},
    ...
    self.assertEqual(
      [],  # XXX maybe [0], see test_group_by_time_sequence_empty_slots_are_returned
...
class TestUnitConversionBackwardCompatibility(BaseTestUnitConversion):
  ...
  def testBackwardCompatibility(self):
    rule = self.getRule(reference='default_delivery_rule')
...
def test_suite():
  ...
  suite.addTest(unittest.defaultTestLoader.loadTestsFromTestCase(TestInventoryAssetPriceValuationMethod))

...
def to_unicode(x, encoding=None):
    ...
    if isinstance(x, six.binary_type):
        return six.u(x, encoding or HTTPRequest.default_encoding)
    return six.u(x)
...
class MessageCatalog(LanguageManager, ObjectManager, SimpleItem):
    ...
    def get_message_key(self, message):
        ...
        if isinstance(message, six.text_type):
            ...
            message = six.u(message, encoding)
        if message in self._messages:
            return message
    ...
    def get_namespace(self, REQUEST):
        ...
        messages = []
        for m, t in self._messages.iteritems():
    ...
    def manage_export(self, x, REQUEST=None, RESPONSE=None):
        ...
        if x == 'locale.pot':
            filename = x
            for k in self._messages.iterkeys():
...
for name, value in message_catalog_aliases.iteritems():

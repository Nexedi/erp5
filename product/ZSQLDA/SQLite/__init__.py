    ...
    def _query(self, query, args=None, allow_reconnect=False):
        ...
        try:
            ...
            if keyword in _transaction_keywords:
                # for no transaction connection, ignore
                # otherwise, it mixed with transaction connection
                # at end, it raise cannot start a transaction within a transaction
                if not self._transactions:
                    return
                # we sometime have a single commit, rollback....
                # which raise cannot commit - no transaction is active
                # use db. instead of execute which corretly handle such case
                if keyword == 'ROLLBACK':
                    self.db.rollback()

...
CREATE TABLE %s (
  ...
  def _getExecutableMessageSet(self, activity_tool, db, message_list):
    ...
    # This provides the SQL database with structurlly simple queries that it
    ...
    to_sql_dict = _DEPENDENCY_TESTER_DICT
    ...
    for message in message_list:
      for (
        ...
        try:
          column_list, _ = to_sql_dict[dependency_name]
        except KeyError:
          continue
        # There are 2 types of dependencies:
        # - monovalued (most), which can accept a vector of values.
        # - 2-valued (after_path_and_method_id and after_tag_and_method_id)
        #   which cannot accept a vector of values.
        if len(column_list) == 1:
          if isinstance(dependency_value, (tuple, list)):
            dependency_value_list = dependency_value
          else:
            dependency_value_list = [dependency_value]
        else:
          dependency_value_list = list(product(*(
            (
              dependency_value_item
              if isinstance(dependency_value_item, (tuple, list)) else
              (dependency_value_item, )
            )
            for dependency_value_item in dependency_value
          )))
        dependency_value_list = [
          x
          for x in dependency_value_list
          if x is not None
        ]
    ...
    for (
      ...
      if not dependency_value_dict:
        ...
        continue
      column_list, to_sql = to_sql_dict[dependency_name]
      if len(column_list) == 1:
        def row2key(row):
          key, = row
          return key
        ...
        # This is the correct condition, but it would be expressed with shorter
      ...
      # Note: the parentheses are here to fool ZMySQLDA into not detecting this
      # as a select query, hence preventing it from applying its annoying
      # default "LIMIT 1000", which we definitely do not want here (as it would
      # cause us to miss dependencies) even though it seems unlikely there will
      # be 1k distinct values blocking a set of activities.
      base_sql_prefix = '(SELECT DISTINCT %s FROM ' % (
        ','.join(column_list),
      )
      base_sql_suffix = ' WHERE processing_node > -10 AND %s)' % (
        dependency_sql,
      )
      for row in db.query(
        ' UNION '.join(

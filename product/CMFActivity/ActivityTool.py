...
class ActivityTool (BaseTool):
    ...
    def getNodeList(self, role=None):
      ...
      if role is None:
        result = [x for x in list(node_dict.keys())]
      else:
        result = [node_id for node_id, node_role in list(node_dict.items()) if node_role == role]
      result.sort()
      return result
    ...
    def getFamilyNodeList(self, family_name):
      ...
      return [
        x
        for x, y in list(self._getNodeFamilyIdDict().items())
        if family_id in y
      ]
    ...
    def hasActivity(self, *args, **kw):
      ...
      return bool(db.query("(%s)" % ") UNION ALL (".join(
        ...
        for activity in activity_dict.values()))[1])
    ...
    def invokeGroup(self, method_id, message_list, activity, merge_duplicate):
      ...
      expanded_object_list = sum(iter(message_dict.values()), [])
      try:
        ...
        for m, expanded_object_list in message_dict.items():
          result_list = []
    ...
    def getSQLTableNameSet(self):
      return [x.sql_table for x in activity_dict.values()]

...
class ERP5Catalog(Folder, Catalog):
  ...
  def _getOb(self, id, default=_MARKER):
    ...
    if default is self._MARKER:
      # Keep the standard Folder._getOb contract (callers such as
      # FilterDict.__getitem__ rely on KeyError, not AttributeError).
      raise KeyError(id)
    return default
  ...
  def _aq_dynamic(self, id):
    ...
    # (a catalog method object is never None).

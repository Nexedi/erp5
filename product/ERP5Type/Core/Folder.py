...
try:
  ...
  class CMFHBTreeFolder(object):
    pass

  class HBTreeFolder2Base(object):
    pass

  class HBTreeFolder2(object):
    pass
...
class Folder(FolderMixIn, CopyContainer, ObjectManager, Base, OFSFolder2, CMFBTreeFolder, CMFHBTreeFolder):
  ...
  def migrateToHBTree(self, migration_generate_id_method=None, new_generate_id_method='_generatePerDayId', bundle_count=10, REQUEST=None):
    ...
    if migration_generate_id_method not in (None, ''):
      ...
      for x in range(old_div(len(self), bundle_count)):
  ...
  def _launchCopyObjectToHBTree(self, tag):
    ...
    BUNDLE_COUNT = 100
    for x in range(old_div(len(id_list), BUNDLE_COUNT)):
  ...
  def recursiveApply(self, filter=dummyFilter, method=None,
    ...
    if include==1 and list(filter(object=self.getObject(),REQUEST=REQUEST)):
    ...
    for o in self.objectValues(): # contentValues sometimes fail in BTreeFolder
      ...
      if list(filter(object=o,REQUEST=REQUEST)):
  ...
  def objectValues(self, spec=None, meta_type=None, portal_type=None,
    ...
    if self._folder_handler == HBTREE_HANDLER:
      ...
      object_list = list(map(self._getOb, self.objectIds(spec)))
  ...
  def contentValues(self, *args, **kw):
    ...
    if filter_kw:
      object_list = list(filter(ContentFilter(**filter_kw), object_list))
    return object_list

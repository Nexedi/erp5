...
@implementer(interfaces.ILocalRoleAssignor)
class LocalRoleAssignorMixIn(object):
    ...
    def updateLocalRolesOnDocument(self, ob, user_name=None, reindex=True, activate_kw=()):
      ...
      if user_name is None:
        ...
        if owner:
          ...
          for user_name, role_list in (ob.__ac_local_roles__ or {}).items():
      ...
      for role_generator in self.getFilteredRoleListFor(ob):
        ...
        for group_id, role_list \
                in role_generator.getLocalRolesFor(ob, user_name).items():
    ...
    def _importRole(self, role_property_dict):
      ...
      for k, v in role_property_dict.items():
        if k == 'condition':
...
@implementer(interfaces.IActionContainer)

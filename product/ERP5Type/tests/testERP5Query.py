...
class TestERP5Query(ERP5TypeTestCase):
  ...
  def _createOrganisationOverwrite(self, **kw):
    ...
    return self.portal.organisation_module.newContent(id=_id, **kw)
  ...
  def _assertQueryKwParsingRenderingMatching(self, query_kw, expected_match_list=None):
    ...
    self.assertEqual(generated_sql, generated_sql_2, "{!r} != {!r}\n\ntraceback_info : {!r}".format(
      generated_sql,
      generated_sql_2,
      {"query_kw": query_kw, "parsed_query": parsed_query, "rendered_search_text": rendered_search_text, "parsed_query_2": parsed_query_2},
    ))
    ...
    resulting_match_list = [r.path for r in catalog(query=parsed_query, portal_type="Organisation", sort_on=[("path", "ascending")])]
  ...
  # XXX Fail because query is invalid, so parse(render(parse(query_kw))) != parse(query_kw)
  ...
  def test_search_text_parsing_rendering_and_matching_with_column_quote_and_2_ending_backslashes(self):
    self._assertQueryKwParsingRenderingMatching({'search_text': 'title:"TestERP5QueryBackslash\\\\"'}, [])  # XXX should it match "022" instead ??
  # XXX Fail because query is invalid, so parse(render(parse(query_kw))) != parse(query_kw)
  ...
  def test_search_text_parsing_rendering_and_matching_with_quote_and_backslash(self):
    self._assertQueryKwParsingRenderingMatching({'search_text': '"TestERP5QueryBackslash\\AndText"'}, [self.organisation_dict["000"]])  # XXX should it match "011" instead ??
  def test_search_text_parsing_rendering_and_matching_with_quote_and_2_backslashes(self):
    self._assertQueryKwParsingRenderingMatching({'search_text': '"TestERP5QueryBackslash\\\\AndText"'}, [self.organisation_dict["011"]])  # XXX should it match "012" instead ??
  def test_search_text_parsing_rendering_and_matching_with_quote_and_3_backslashes(self):
    self._assertQueryKwParsingRenderingMatching({'search_text': '"TestERP5QueryBackslash\\\\\\AndText"'}, [self.organisation_dict["011"]])  # XXX should it match "013" instead ??
  ...
  def test_search_text_parsing_rendering_and_matching_with_quote_and_ending_backslash(self):
    self._assertQueryKwParsingRenderingMatching({'search_text': '"TestERP5QueryBackslash\\"'},
                                                [self.organisation_dict[k] for k in self.organisation_dict.keys() if k.startswith("00") or k.startswith("01") or k.startswith("02")])  # XXX should it really match "000" ??
  def test_search_text_parsing_rendering_and_matching_with_quote_and_2_ending_backslashes(self):
    self._assertQueryKwParsingRenderingMatching({'search_text': '"TestERP5QueryBackslash\\\\"'},
                                                [self.organisation_dict[k] for k in self.organisation_dict.keys() if k.startswith("01") or k.startswith("02")])  # XXX should it really match "021" ??
  # XXX Fail because query is invalid, so parse(render(parse(query_kw))) != parse(query_kw)
  def test_search_text_parsing_rendering_and_matching_with_quote_and_3_ending_backslashes(self):
    self._assertQueryKwParsingRenderingMatching({'search_text': '"TestERP5QueryBackslash\\\\\\"'},
                                                [self.organisation_dict[k] for k in self.organisation_dict.keys() if k.startswith("01") or k.startswith("02")])  # XXX should it really match "021" and "022" ??
  ...
  # XXX Fails because parse(render(parse(query_kw))) != parse(query_kw)

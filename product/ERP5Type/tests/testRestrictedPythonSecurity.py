...
class TestRestrictedPythonSecurity(ERP5TypeTestCase):
  ...
  # Now it is not allowed so the following test is disabled
  def _testDecimalClassMethod(self):
    self.createAndRunScript('import decimal',
                            'return decimal.Decimal.from_float(3.3)')

  # Now it is not allowed so the test is disabled
  def _testDictClassMethod(self):

...
if not IS_COMPONENT_PY2_LOADER:
  ...
  class ComponentModuleLoader(ComponentModuleInspectLoaderMixin):
    ...
    the minimum.
    """
    def _getVersionPackage(self, package_fullname, version):
      """
      ...
      if version_package is None:
        version_package_name = package_fullname + '.' + version
    ...
    def _load_module_unlocked(self, fullname):
      ...
      if name.endswith('_version'):
        ...
        return (version in site.getVersionPriorityNameList() and
                self._getVersionPackage(package_fullname, version) or None)
      ...
      version_package = self._getVersionPackage(package_fullname, version)

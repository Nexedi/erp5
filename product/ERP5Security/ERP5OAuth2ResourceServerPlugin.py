...
class ERP5OAuth2ResourceServerPlugin(BasePlugin):
  ...
  def __updateAccessTokenSignatureKeyList(self, request):
    ...
    for connector_value in self.__iterClientConnectorValue():
      log("__updateAccessTokenSignatureKeyList: connector_value=%s" % connector_value)
  ...
  def __decodeToken(
    ...
    try:
      ...
      log("decodeToken/%s: raised err =  %s" % (request["ACTUAL_URL"], e))
      raise

    log("decodeToken/%s: token=%s" % (request["ACTUAL_URL"], token))
    if token is None and can_update_key:
      self.__updateAccessTokenSignatureKeyList(request=request)
      token = self.__checkTokenSignature(access_token)
    log("decodeToken/%s: token(2)=%s" % (request["ACTUAL_URL"], token))
    if token is None:
      return
    log("decodeToken/%s: SUCESS __checkTokenSignature" % request["ACTUAL_URL"])
    # Non-standard claims
    if not isAddressInNetworkList(
      ...
      return
    log("decodeToken/%s: SUCESS isAddressInNetworkList" % request["ACTUAL_URL"])
    # JWT is known valid. Access its content.
    token_payload = decodeAccessTokenPayload(
      ensure_ascii(token[JWT_PAYLOAD_KEY]),
    )
    ...
    if self.__getWebServiceValue(
      ...
      return
    log("decodeToken/%s: SUCESS JWT" % request["ACTUAL_URL"])
    return {
  ...
  def __extractAuthorisation(self, request):
    ...
    if authorisation_scheme != 'Bearer':
      return
    log("extractAuthoristaion: %s" % authorisation_value)
    ...
    log("extractAuthoristaion: result=%s" % result)
  ...
  def extractCredentials(self, request):
    ...
    if request.REQUEST_METHOD == "POST":
      ...
      log("extractCredentials/%s: start, AUTH = %s" % (reqURI, request['HTTP_AUTHORIZATION']))
      log("extractCredentials/%s: start, _auth = %s" % (reqURI, request._auth))
    # else:
    #   log("extractCredentials: NOT POST REQUEST req = \n%s" % request)
    ...
    if request.REQUEST_METHOD == "POST":
      log("extractCredentials/%s: request_private_dict = '%s'" % (reqURI, request_private_dict))
    if not request_private_dict:
      for private_dict in (
        ...
        # if request.REQUEST_METHOD == "POST":
        #   log("extractCredentials: request_private_dict loop = '%s'" % private_dict)
        if private_dict is not None:
          ...
          break
    if request.REQUEST_METHOD == "POST":
      log("extractCredentials/%s: request_private_dict (2) = '%s'" % (reqURI, request_private_dict))
    if not request_private_dict:
      return

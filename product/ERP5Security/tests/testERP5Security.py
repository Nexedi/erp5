...
class TestUserManagement(ERP5TypeTestCase):
  ...
  def test_scriptGenerationByAlarm(self):
    ...
    # AccessControl.setImplementation('python')
    ...
    self.portal.Base_checkPermission('person_module', 'View')
...
if getattr(container, script_id, None) is None:
  ...
  def testSkipOwnershipChecking(self):
    import pdb; pdb.set_trace()

    import AccessControl
    # AccessControl.setImplementation('python')
    # AccessControl.setDefaultBehaviors(
    #   False, #not self.cfg.skip_ownership_checking,
    #   True, #not self.cfg.skip_authentication_checking,
    #   False) # self.cfg.verbose_security)
    ...
    # XXX: Another way is to do newSecurityManager(None, system) as
    # UnrestrictedMethod does
    script = UnrestrictedMethod(createZODBPythonScript)(
      self.portal.portal_skins.custom,
      "TestSkipOwnershipChecking", '',
      "return context.getPortalObject().person_module.contentValues()")

    import transaction
    transaction.commit()

    from AccessControl.SpecialUsers import system
    self.assertEquals(script.owner_info()['id'], system.getUserName())

    self.login()
    self.portal.portal_skins.custom.TestSkipOwnershipChecking()

...
class MonovaluedComparisonOperator(ComparisonOperatorBase):
  def renderValue(self, value_list, renderer):
    ...
    return self._renderValue(value_list, renderer=renderer)
  ...
  def render(self, column, value_list, renderer):
    ...
    if isinstance(value_list, list_type_list):
      try:
        ...
        raise ValueError('%r: value_list must not contain more than one item. Got %r' % (self, value_list))
    return self._render(column, value_list, renderer = renderer)
...
class MultivaluedComparisonOperator(ComparisonOperatorBase):
  def renderValue(self, value_list, renderer):
    ...
    return '(%s)' % ', '.join(self._renderValue(v, renderer=renderer) for v in value_list)
  ...
  def render(self, column, value_list, renderer):
    ...
    if not isinstance(value_list, list_type_list) or len(value_list) < 2:
      raise ValueError('%r: value_list must be a list of more than one item. Got %r' % (self, value_list))
    return column, '(%s)' % ', '.join(self._renderValue(v, renderer=renderer) for v in value_list)
...
class MroongaComparisonOperator(MatchComparisonOperator):
  ...
  def renderValue(self, value_list, renderer):
    ...
    if self.force_boolean:
      ...
      return self._renderValue(self._escape(fulltext_query), renderer=renderer)
    else:
      match_query_list = []
      ...
      return self._renderValue(fulltext_query, renderer=renderer)

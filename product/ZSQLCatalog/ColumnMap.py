...
class ColumnMap(object):
  ...
  def _mapColumns(self, column_table_map, table_usage_dict, column_name_set, group, vote_result_dict):
    ...
    for column_name, candidate_dict in vote_result_dict.items():
      # candidate_dict is never empty
      max_score = 0
      for table_name, score in candidate_dict.items():
        if score > max_score:
          max_score = score
          best_count = 0
      ...
      for table_name, column_set in table_usage_dict.items():
    ...
    weighted_table_list = sorted(iter(table_usage_dict.items()), key=table_weight)
    ...
    for column_name, table_name in mapping_dict.items():
  ...
  def build(self, sql_catalog):
    ...
    for group, column_set in self.registry.items():
      ...
      for table_vote_method in table_vote_method_list:
        ...
        if isinstance(vote_dict, dict):
          for column, table in vote_dict.items():
    ...
    for (group, table_name), alias in self.table_alias_dict.items():
      if alias is None:
    ...
    if MAPPING_TRACE:
      ...
      #   Value: table alias
      summary_dict = {}
      for (group, column), table_name in self.column_map.items():
      ...
      for (group, table_name), table_alias in self.table_alias_dict.items():
      ...
      for group, (column_dict, table_dict) in summary_dict.items():
        LOG('ColumnMap', INFO, 'Group %r:' % (group, ))
        LOG('ColumnMap', INFO, ' Columns:')
        for column, table_name in column_dict.items():
          LOG('ColumnMap', INFO, '  %r from table %r' % (column, table_name))
        LOG('ColumnMap', INFO, ' Tables:')
        for table_name, table_alias in table_dict.items():

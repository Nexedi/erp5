...
class RPCRetry(object):
    ...
    def _RPC(self, func_id, args=()):
        try_tool = False
        try:
          result = getattr(self._proxy, func_id)(*args)
        except Exception as e:
          fault_string = getattr(e, 'faultString', None)
          if func_id == 'createTestResult' and fault_string is not None \
            and 'AttributeError' in fault_string \
            and 'portal_task_distribution_tool' in fault_string:
              # The Distributor on the server is incorrect, try
              # to use tool proxy
              self._logger.warning(
                'The distributor failed with error. Fall-back to tool on the '
                'server for the function %r' % (func_id,), exc_info=1)
              try_tool = True
          else:
              raise
        if not try_tool and func_id == 'createTestResult' and result is None:
            # The Distributor on the server is incorrect, try
            # to use tool proxy
            self._logger.warning(
              'The distributor returned None. Fall-back to tool on the '
              'server for the function %r' % (func_id,))
            try_tool = True
        if try_tool:
            result = getattr(self._tool_proxy, func_id)(*args)
        return result

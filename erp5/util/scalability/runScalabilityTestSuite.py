...
TEST_METRIC_TIME_INTERVAL = 30#60*3
...
class ScalabilityLauncher(object):
  ...
  def run(self):
    ...
    # XXX for DEBUG ######### #ROQUE5
    user_quantity = 2 #user_quantity *2 #users[current_test_number]
    test_duration = 20 #user_quantity * 30
    SCALABILITY_TEST_DURATION = 35
    test_suite_list = ['createPerson']
    #test_suite_list = ['createSaleOrder']
    #########################
    ...
    for index, command in enumerate(command_list, start=1):
      ...
      test_thread.start()
    # Sleep
    self.log("Going to sleep for %s seconds." % str(SCALABILITY_TEST_DURATION))
    time.sleep(SCALABILITY_TEST_DURATION)
    ...
    self.log("metric_list: " + str(metric_list))
    ...
    if not test_output:
      ...
      if metric_error:
        ...
        error_message = "Metric thread couldn't get any metric after %d seconds" % MAX_GETTING_CONNECTION_TIME
      error_message_set.add(error_message)
    else:
      self.log("metric_error: " + str(metric_thread.getErrorMessage()))
